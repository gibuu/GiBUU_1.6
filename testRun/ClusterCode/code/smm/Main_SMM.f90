...
!*************************************************************************
    ...
    integer :: i,j,m,iii
    integer :: Masse,Ladung,dummy,SourceType,TypeOfSource
    real    :: Exc,Erad,x,y,z,vx,vy,vz,bb,impactParameter
    real    :: Ebind,Etot,ProjectileVelo
    ...
    Loop_over_SubEvents : do m=1,SubEvents
       ...
       Loop_over_Ensemples : do i=1,NumEnsemples
          ...
          Loop_over_Sources : do j=1,NumSources(i)
             ...
             read(2,*) dummy,Masse,Ladung,x,y,z,vx,vy,vz,Etot,Erad,bb,dummy,dummy
                ...
                if (Masse > 2) then
                   if (.not. GetExcBalance) then 
                      ...
                      call ExcFromBalance(i,beamID,beamEnergy,A_init,A_res, & 
                           &              vx,vy,vz,Etot,ParticleVector,Exc)
                   end if
                else
...
!                  & (Etot-0.938)*1000.,Erad*1000.,Exc*1000.
       ...
       Loop_over_Ensemples2 : do i=1,NumEnsemples
          ...
          if (Get_Hyp) & 
               & call MultiFragmentationHyp(m,i,TheSource, & 
               & FragmentVector,ParticleVector)

          if (m==SubEvents .and. i==NumEnsemples) printFlag=.true.

          !print FragmentVector
          if (printFV_Flag) call printFragmentVector(m,numEnsemples,FragmentVector)
          ...
          !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                call rapidityDistribution(SubEvents,numEnsemples,SMM_Events, & 
                     & i,FragmentVector,ParticleVector,printFlag,impactParameter)
          ...
          if (Get_GiBUUvec) & 
               & call producedParticles(SubEvents,numEnsemples,SMM_Events, & 
               &                        i,ParticleVector, & 
...
!***************************************************************************
  ...
  subroutine ExcFromBalance(inum,beamID,beamEnergy,A_init,A_res, & 
    ...
    real, intent(in) :: beamEnergy,vx,vy,vz,Etot
    real, intent(out) :: Exc
    !-----------------------------------------------------------------------
    ! local variables
    !-----------------------------------------------------------------------
