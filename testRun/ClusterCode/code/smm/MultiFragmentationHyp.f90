...
contains 
  ...
  !*************************************************************************
  subroutine MultiFragmentationHyp(SubRuns,NumEns,TheSource,FV,PV)

    implicit none
    !-----------------------------------------------------------------------
    ! Input-Output variables
    !-----------------------------------------------------------------------
    integer,                        intent(in)    :: SubRuns,NumEns
    ...
    do j=1,Size(TheSource,dim=2)
       ...
       SMMevents2 : do i=1,size(FV,dim=1)
          SMMclusters2 : do jj=1,size(FV,dim=2)
             fragment    = FV(i,jj)
             realParticle(:) = PV(NumEns,:)
             call ToyModel(NumPart,SourceSpect,fragment,realParticle)
             FV(i,jj)     = fragment
  ...
  subroutine FormHypFragments(NumPart,SourceSpect,fragment,realParticle)
    ...
    real, dimension(1:3) :: rdist,PosNew,MomPart,MomFra,Pcoala,rhyp
...
!    fragment%MassNumber  = fragment%MassNumber + HypFound
  ...
  ! 
  !*************************************************************************
  subroutine ToyModel(NumPart,SourceSpect,fragment,realParticle)
