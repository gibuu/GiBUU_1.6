...
!         write(*,*)
    ...
    subroutine GetDecayChannel(FMass,ZMass,Ex,betax,betay,betaz,& !--> Input
      ...
      Channels : do k=1,size(Elements(Index)%Ejectile)
         ...
         call DecayWidth(Ai,Zi,Aj,Zj,Ad,Zd,MassExAi,MassExAj,MassExAd, & 
              &          SpinAj,Ex,Width_k)
         Width(k)= Width_k
...
!         write(*,*) 'this should not happened...',Normalization
    ...
    !***********************************************************************
    subroutine DecayWidth(Ai,Zi,Aj,Zj,Ad,Zd,MassExAi,MassExAj,MassExAd, & 
         &                SpinAj,Ex,Width_k)
      use initGEM, only : Nuclide,Elements,NucPosition
      implicit none
      !---------------------------------------------------------------------
      ...
      real,    intent(in) :: MassExAi,MassExAj,MassExAd,SpinAj,Ex
      !---------------------------------------------------------------------
      !Input-Output-Variables
      !---------------------------------------------------------------------
