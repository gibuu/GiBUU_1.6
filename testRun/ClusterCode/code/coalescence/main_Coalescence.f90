...
!      integer :: ios,ior,ioe,idp,iso,subev,status,time,it,SourceIndex
      ...
      !------------------------------------------------------------------------*
1000  format(78('*'),/,9('*'),& 
           & ' START OF PHASE-SPACE COALESCENCE + STATISTICAL EVAPORATION ',& 
           & 9('*'),/,78('*'))
1001  format(78('#'),/,10('#'),' SubEvents    = ',i5,/, & 
           &           10('#'),' NumEnsemples = ',i5,/,78('#'))
...
!                     read(1,*,iostat=ior) idp,SourceIndex,iso,mass,x,y,z,px,py,pz,ActualEvent,subev
               ...
               if(Get_Analysis) then
                  call MainAnalysis(i,j,FragmentVector,ParticleVector,Get_Flow,Get_Zdist, & 
                       SubEvents,NumEnsemples,impactParameter)
               endif
               !----------------------------------------------------------------
               if (Get_Output) call WriteGlobalVector(i,j,FragmentVector)
               !----------------------------------------------------------------
      ...
      !-------------------------------------------------------------------------*
222   format(3(1x,i5),1x,6(f8.4,1x))
      !-------------------------------------------------------------------------*
    !****************************************************************************
