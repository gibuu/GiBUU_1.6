...
contains
  ...
  logical function WriteHeaderMC (H, iFile, mul, FinalLineBreak)
    ...
    S0(0) = sum(H%yVal(1:iBinMax,:))
    S0(1:nCh) = sum(H%yVal(1:iBinMax,:),dim=1)
    ...
    write(iFile,'(a)') '###'
    write(iFile,'(a,a)') '### Column  1 (X-Value): ', trim(H%xDesc)
    write(iFile,'(a)') '### Column  2 (Y-Total): sum of all following channels'
    if (allocated(H%yDesc)) then
      do iCh=1,nCh
        write(iFile,'(a,i2,a,i2,a,a)') '### Column ',iCh+2,' (Y-Channel ',iCh,'): ', trim(H%yDesc(iCh))
      end do
    end if
    write(iFile,'(a)') '###'

    write(f,'(a,i2,a)') '(a,',nCh+1,'ES12.4)'
    write(iFile,'(a)') '### Integrated Values per Channel:'
    write(iFile,fmt=f) '###  I0  ', S0*mulFak
    write(iFile,'(a)') '### Averaged Values per Channel:'
    write(iFile,fmt=f) '###  I1  ', S1
...
!   end subroutine WriteHistMC_Spline
  ...
  subroutine WriteHistMC_Gauss (H, file, width_in)
    ...
    type(histogramMC) :: H
    character*(*), intent(in) :: file
    real, intent(in), optional :: width_in
  ...
  function ReadHistMC (H, file, w) result (success)
    ...
    integer :: i,ios, head, numbins, nCh
    real :: x,s
    character(20) :: f
    ...
    do i=1,head
      ...
      if (i==2) then
        ...
        H%xDesc = line(index(line,":")+2:)
      else if (i>11 .and. i<=11+nCh) then
        H%yDesc(i-11)=line(index(line,":")+2:)
      end if
    end do
