...
contains
  ...
  subroutine readinput(nuXsectionmode)
    ...
    case (1)
       write(*,*) 'pi N background (resonances are substructed) is taken MAID-like'
    case default
  ...
  subroutine SetXsecMC(eNev, flux_enu, XsectionMode)
    ...
    if (.not.flagOK) then
       MC_weight = 0.0
       !write(*,*) 'after eNev_init_nuStep3c flakOK is false. MC_weight = 0.0'
    else
       Eprime=eNev%lepton_out%momentum(0)
       ! from dsi/dE1dcostheta to dsi/dxdy
       MC_weight = (MC_y*PK**2)/(Ein *sqrt(Eprime**2-eNev%lepton_out%mass**2)*eNev%nucleon%momentum(0))
       !write(*,*) 'MC_weight=',MC_weight
    end if
  ...
  subroutine Xsec_SigmaMC_W(eNev, IP, raiseFlag, raiseVal, OutPart,sig,flux_enu)
    ...
    PK = SP(eNev%lepton_in%momentum,eNev%nucleon%momentum)
    !write(*,*) 'PK=',PK, '   W=',W, '   MC_y=',MC_y, '    MC_weight=',MC_weight
    sig = sig * MC_weight *W /(PK *MC_y)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    case (:31)
       ...
       !invariantMasscut
       if(invMassSQ.gt.invariantMasscut**2) return
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    case(onePionCH_n,onePionCH_p)
       ...
       case(0) !HNV model
          ...
          !invariantMasscut
          invMassSQ=SP(p_out+pion_momentum_out,p_out+pion_momentum_out)
          if(invMassSQ.gt.invariantMasscut**2) sig=0.

          mass_out=mN

       case (1) !MAID like

          !momentum of outgoing hadrons
          invMassSQ=SP(p_in+k_in-k_out,p_in+k_in-k_out)
          if(invMassSQ.le.0.) return !reaction not possible

          !invariantMasscut
          if(invMassSQ.gt.invariantMasscut**2 &
               & .or.invMassSQ.gt.invariantMasscut_BG**2) return
