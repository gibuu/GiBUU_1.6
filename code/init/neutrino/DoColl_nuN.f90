...
!!$    write(*,'(A," = ",1P,e13.5)') 'pQpQ',pQpQ
  ...
  subroutine CalcAnaEstimate(eNev, sigma)

    use CallStack, only: TRACEBACK
    use eN_event
    use minkowski, only : abs4Sq
    use constants, only : pi, GF

    type(electronNucleon_event), intent(inout):: eNev
    real, intent(out) :: sigma ! = dsigma/dxdy in mb
    ...
    if (eNev%nucleon%charge .gt. 0) then ! *** proton ***
       if (.not.eNev%lepton_in%antiparticle) then
  ...
  end subroutine CalcAnaEstimate
