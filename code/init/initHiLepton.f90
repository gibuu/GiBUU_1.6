...
module initHiLepton
  ...
  PUBLIC :: InitHiLeptonInduced, AccWeight, GETiExperiment, GetPhotonKin
  PUBLIC :: GetEnergies
  ...
  real, save :: numin,numax,costmin,costmax,ymax,Q2max,Q2min,smin,xBmin
  real, save :: maxw
...
contains
  ...
  subroutine InitHiLeptonInduced(rParts,pParts,targetNuc)
    ...
    type(histogram), save :: h_RhoTP, h_RhoTF
...
!!$       call CreateHist2D(h2D_nuQ2(4),"nuQ2: 999 [misc]",P1,P2,dP,.true.)
       ...
       call CreateHist(h_RhoTP, "t_P (Rho0)", 0.0,20.0,0.1)
...
!          write(*,'(i3,A,1P,10g12.5)') ipart,'  nu,Q2,Ebeam,eps,W = ',nu,Q2,Ebeam,eps,W
          ...
          RetryLoop: do
             ...
             call eventGen_eN_HiEnergy(eNev_RunData,iEns*1000+iPart,scaleVMD,outPart,channel,flagOK,XS_tot,XS_Arr )
             
             if (XS_tot.lt.1e-10) then
...
!!$                cycle ParticleLoop
             ...
             call conservation(eNev_RunData,outPart,flagOK)
             if (.not.flagOK) then
             ...
             call checkPauli(outPart, rParts, flagOK)
             if (.not.flagOK) then
...
!!$          end select
    ...
    if (DoStatistics) then
       ...
       call WriteHist2D_Gnuplot(h2D_shad,DoAve=.true.,MaxVal=-1.0,file='HiLep.Shadowing.dat',dump=.true.)

       call WriteHist2D_Gnuplot(h2D_Pos,add=add0,mul=mul0,file='HiLep.Pos.dat',dump=.true.)

       call WriteHist2D_Gnuplot(h2D_nuQ2_Eps,DoAve=.true.,MaxVal=-1.0,file='HiLep.nuQ2.Eps.dat',dump=.true.)
       call WriteHist2D_Gnuplot(h2D_nuQ2_EpsR,DoAve=.true.,MaxVal=-1.0,file='HiLep.nuQ2.EpsR.dat',dump=.true.)
       
       call WriteHist2D_Gnuplot(h2D_nuQ2_flux,add=add0,mul=mul0,file='HiLep.nuQ2.Flux.dat',dump=.true.)
       call WriteHist2D_Gnuplot(h2D_nuQ2_fluxW,add=add0,mul=mul0,file='HiLep.nuQ2.FluxW.dat',dump=.true.)
       do i=0,4
          call WriteHist2D_Gnuplot(h2D_nuQ2(i),add=add0,mul=mul0,file='HiLep.nuQ2.'//intTochar(i)//'.dat',dump=.true.)
       end do

       call WriteHist(h_RhoTP,add=add0,mul=mul0,file='HiLep.RhoTP.dat',dump=.true.)
       call WriteHist(h_RhoTF,add=add0,mul=mul0,file='HiLep.RhoTF.dat',dump=.true.)
       
       call WriteHist2D_Gnuplot(h2D_WvsWfree,add=add0,mul=mul1,file='HiLep.WvsWfree.dat',dump=.true.)
       call WriteHist2D_Gnuplot(h2D_WvsW0,add=add0,mul=mul1,file='HiLep.WvsW0.dat',dump=.true.)
       call WriteHist(h_pFermi,add=add0,mul=mul1,file='HiLep.pFermi.dat',dump=.true.)
...
!!$  end subroutine determineHiPhoton
  ...
  !
  ! NOTES
  ! * since we changed the definition according the MC selction, the 
  ...
  !   first runs. See the file "initHiLepton.AdjustMaxW.txt" for the largest
...
!       call write_electronNucleon_event(eNev, .false., .true.)
  ...
  contains
    subroutine WriteAdjustMaxW(r)
      ...
      if (r.ge.rmax) then
         rmax = r + 1e-10
         open(121, file="initHiLepton.AdjustMaxW.txt", status='unknown')
         rewind(121)
         write(121,*) r
         write(121,*)
         close(121)
...
!c     0 to 1. It does not have to equal 4pi when integrated over.
  ...
  !*************************************************************************
  !****s* initHiLepton/conservation
  ! NAME
  ! subroutine conservation(eNev,outPart,flagOK)
  !
  ! PURPOSE
  ...
  !
  ! OUTPUT
  ! * logical :: flagOK -- .true. if all checks okay
  !*************************************************************************
  subroutine conservation(eNev,outPart,flagOK)
