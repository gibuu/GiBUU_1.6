...
contains
  ...
  subroutine eventGen_eN_lowEnergy(eN,doC,whichRes,OutPart,channel,flagOK,XS,XS_Arr)
    ...
    type(particle)                     :: OutPart_Res, OutPart_QE
    ...
    if (doC(iC_2p2hDelta)) call init_2p2hQE(eN,OutPart_2p2hDelta,weights(iC_2p2hDelta))
    ...
    Case(1) ! ===== QE Event =====
       OutPart(1)=OutPart_QE
       channel=OutPart_QE%ID

    Case(2) ! ===== Resonance Event =====
       OutPart(1)=OutPart_Res
       channel=OutPart_Res%ID
    ...
    Case Default
       STOP 'Wrong Monte-Carlo Decision in eventGen_eN_lowEnergy'
    End Select

    flagOK=.true.
    OutPart%perweight=totalWeight

    call checkEvent(eN,OutPart,flagOK,channel)
  ...
  !*************************************************************************
  !****s* eventGenerator_eN_lowEnergy/checkEvent
  ! NAME
  ...
  subroutine checkEvent(eN,f,flagOK,channel)
    ...
    mom_in =eN%nucleon%momentum+eN%boson%momentum
    do j=0,3
...
!     type(medium)         :: mediumDUMMY    ! use as DUMMY
   ...
   ! Check for Monte-Carlo-Integrations:
    ...
    pionChargeLoop: do pionCharge=eN%nucleon%charge-1,eN%nucleon%charge
       ...
       if (.not.modeBckGrnd) then
          ...
          if(twoRoots) then
             write(*,*) 'ERROR in init_1Pi. Two roots in getKinematics_eN!!!'
             STOP
          end if
  ...
  subroutine init_QE(eN,OutPart,XS)
    ...
    ! Generate Event
    call generateEvent_1Body(OutPart,eN%nucleon,pf(:),sigma,nucleon,baremass_out)
    flagOK=.true.

    ! Take care of offshellness
    call setOffShellParameter(OutPart,flagOK)
    if (.not.flagOK) OutPart%perweight=0.0
    XS = OutPart%perweight
  ...
  subroutine init_Res(eN,useRes,OutPart,XS)
    ...
    real, dimension (nucleon+1:nucleon+nres,0:3)  :: pf
    integer                                   :: resID
    ...
    call generateEvent_1Body(OutPart,eN%nucleon,&
