...
contains
  ...
  subroutine eventGen_eN_HiEnergy(eNev,firstevent,scaleVMD,OutPart,channel,FlagOK,XS_tot,XS_Arr)
    ...
    ! dsigma/dE'dOmega (in mub/MeV) to dsigma/dnudQ2 (in mub/GeV^3)
    ...
    if (DoToyModel_rho) then
       ...
       call eventGen_eN_lowEnergy(eNev,doC,useRes, outPart,channel,flagOK,XS_tot)
    ...
    if (Wfree.lt.HighEnergyThreshold) then
       if (DoLowEv) then
          ...
          doC = (/.false.,allowRes,allow1Pi,allow2piBack,allowDIS,allowVMDrho,.false.,.false./)
          call eventGen_eN_lowEnergy(eNev,doC,useRes, outPart,channel,flagOK,XS_tot)
