...
5200 FORMAT (I3,1X,A28,': ',1P,D10.3,' mb')
  ...
  ! If file!=0 also output is written to file.
  !
  ! This returns d^sigma/dy dQ^2 / flux^T,
  ! i.e. sigma_T(y,Q^2) + eps * sigma_L(y,Q^2) in mb
  !
  ! (adapted form PYSTAT)
  !
  ! INPUTS
  ...
  subroutine CollectXS_class(XS, file)
    ...
    ! transverse flux, as used in PYGAGA:
...
!!$    ! correction factor dx -> dyB:
!!$    ! (W^2-M^2)/(W^2-M^2+Q^2) (m_e ignored)
!!$
!!$    fT_h = fT_h * (VINT(2)-0.938**2)/(VINT(2)-0.938**2+VINT(307))

!!$    ! convert from dyB to dnu
!!$    fT_h = fT_h * nu/yB
    ...
    if (MINT(121).eq.4.and.MSTP(14).eq.30) then
       ...
       do i=1,4
          ...
          XS(0) = XS(0)+XS(i)
          if (file.gt.0) write(file,5200) PROGP4(i),XS(i)
       enddo
       ...
       if (file.gt.0) then
          do i=1,4
             write(file,5200) PROGP4(i),XS(i)
          enddo
          write(file,5200) PROGP4(0),XS(0)
       endif
