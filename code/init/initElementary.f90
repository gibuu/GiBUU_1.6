...
contains
  ...
  contains 
    ...
    ! NOTES
    ! * If impactParameter is choosen to be less than zero, than the 
    !   impact parameter is choosen by a Monte-Carlo-decision.
    ! * 2-nd particle is always initialized at zero (already done
    ...
    subroutine setPosition
      ...
      if (impactParameter < 0.) then
         if (AutoAdjustB) b =  AdjustImpact(teilchen(j,1:2))
      endif

      ratio = getRatio((/teilchen(j,1),teilchen(j,2)/)) ! Medium modification factor
      siggeo = 10.0*pi * b**2  * ratio

      if(impactParameter < 0.) then
         b = b * sqrt(ratio) * sqrt(rn())
      end if
    ...
    real function AdjustImpact(pair)
      ...
      real :: sigmaLbar,sigmaSbar,sigmaXiBar,sigmaJPsi, b0
