...
contains
  ...
  real function get_k_abs(ak,bk,ck,q,initNuc,pionCharge,nucleon_out_charge,&
    ...
    k_abs(1)=low+0.2
    call set_k(k_abs(1),k,initNuc,pionCharge,betaToCF)
    ...
    k_abs(2)=low+0.5
    call set_k(k_abs(2),k,initNuc,pionCharge,betaToCF)
    ...
    do 
       if(Abs(f(2)-f(1)).eq.0) then
          ...
          f(2)=error_energyConservation(k,q,initNuc,pionCharge,nucleon_out_charge,betaToCF,k_abs(2),low,high)
          if(debugFlag) then
             write(*,*)'Error in get_k_abs. Derivative to small:', f(1:2),f(2)-f(1),numRestarts
             call errorMessage
       ...
       if(counter.gt.maxCounter) then
          if(debugFlag) then
             write(*,*)'Error in get_k_abs.  counter.gt.maxCounter'
             call errorMessage
          end if
          get_k_abs=1111111111.
          success=.false.
  ...
  contains
    ...
    subroutine scan()
      ...
      do
         x=x+0.01
         call set_k(x,k,initNuc,pionCharge,betaToCF)
    ...
    subroutine set_k(absolut,k,initNuc,pionCharge,betaToCF)
