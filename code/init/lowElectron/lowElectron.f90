...
module lowElectron
  ...
  !
  ! PURPOSE
  ! Decides which set of variables is used to determine the final electron energy energy_lf and
  ! the step size delta_energy_lf
...
contains
  ...
  subroutine init_lowElectron(realParticles,pertParticles,raiseEnergy,targetNuc)
    ...
    use Electron_origin , only : le_whichOrigin
    ...
    write(40,'(6E20.5)') energy_li,energy_lf,QSquared,theta_lf,&
    ...
    ensembleLoop: do j=lbound(realParticles,dim=2),ubound(realParticles,dim=2)
       indexLoop: do i=lbound(realParticles,dim=1),ubound(realParticles,dim=1)
          ...
          call eventGen_eN_lowEnergy(eN,doC,useRes, finalState,channel,successFlag,xsection)
          ...
          call checkPauli(finalState,realParticles,successFlag)
          if(.not.successFlag) cycle indexLoop
