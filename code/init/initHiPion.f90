...
module initHiPion
  ...
  PUBLIC :: getTotalPerweight, getBetaFak !, getNumberPions
  ...
  !*************************************************************************
  !****g* initHiPion/DoProton
  ! SOURCE
  !
  logical,save   :: DoProton = .FALSE.
  !
  ! PURPOSE
  ! If .false., projectile is a pion.
  ! If .true., projectile is a proton.
  !*************************************************************************
...
!    stop
  ...
  contains
    ...
    subroutine initInput
      ...
      ! * NucCharge
      !*************************************************************************
      NAMELIST /HiPionNucleus/ distance, impact_parameter, Charge, numberPions, &
           ekin_lab, p_lab, doPerturbativeInit, &
           DoOnlyOne, DoProton, minimumMomentum, useHermesPythiaPars, NucCharge
      ...
      write(*,*) '  Number of projectiles per ensemble: ',numberPions
      write(*,*) '  Charge of projectile:               ',Charge
      write(*,*) '  Pion is Proton:                 ',DoProton
...
!      end do
      ...
      call Dilep_Init (ekin_lab,numberPions,beta=betaCM2Lab)
    ...
    subroutine setKinematics(part)
      ...
      if (DoProton) then
         part%ID=nucleon
         part%charge=Charge
         part%antiparticle= (Charge < 0) ! Antiproton is antiparticle!
         part%mass=mN
      else
         part%ID=pion
         part%charge=Charge
...
!      real :: minimalDistance=2.52        ! sqrt(maximal crossection of pion and nucleus/pi)
      ...
      if (impact_parameter.ge.0.) then
         ...
         pertPart(j,index)%perweight=1./float(totalNumPions)
...
!!$             call WriteParticle(99,iEnsemble,j,pair(1))
             ...
             call collide_2body (pair, finalState, time, flagOK, HiEnergyFlag, HiEnergyType, EventWeight)
             ...
             call checkPauli(finalState, realPart, flagOK)
...
!             write(99,*) 'HiEnergyType=',HiEnergyType
             ...
             finalState%perweight=pair(2)%perweight * EventWeight
             ...
             call ReportEventNumber(pair, finalState, finalState(1)%event, &
                  & time,212,HiEnergyType,EventWeight)
...
!             write(99,*) 'HiEnergyType=',HiEnergyType
             ...
             call ReportEventNumber(pair, finalState, &
                  & finalState(1)%event,time,212,HiEnergyType,EventWeight)
