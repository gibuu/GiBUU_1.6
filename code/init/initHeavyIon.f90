...
module initHeavyIon
  ...
  ! PURPOSE
  ! This switch provides impact parameter distributions for trigger-biased setups.
  ! Only used for impact_parameter < 0.
  ! Possible values:
  ! * 0: minimum bias (default)
  ! * 1: HADES C+C at 1 AGeV
  ! * 2: HADES C+C at 2 AGeV
...
contains
  ...
  subroutine initHeavyIonCollision (targetNuc, projectileNuc)  
    ...
    if (impact_parameter >= 0.) then
      ...
      case (0)
        ...
        b = bmax * sqrt(rn())
      case (1,2,3)
        ! use HADES trigger-biased centrality distributions
        do
          b = bmax * sqrt(rn())
          x = rn()
          if (x < impact_HADES(b,impact_profile)) exit  ! accept
        end do
      case default
  ...
  contains
    ...
    real function impact_HADES (b, i)
      ...
      integer, intent(in) :: i   ! select reaction: 1 = CC1, 2 = CC2, 3 = ArKCl

      real, parameter :: b0(1:3) = (/ 3.705, 4.034, 4.922 /)  ! Woods-Saxon parameters for HADES centrality selection
      real, parameter :: db(1:3) = (/ 0.767, 0.749, 0.601 /)
