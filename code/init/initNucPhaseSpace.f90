...
contains
  ...
  subroutine init
    ...
    use baryonPotentialModule, only: getsymmetriePotFlag_baryon, getPotentialEQSType
    ...
    if(.not.getRMF_flag()) then
        if (determine_Fermi_momentum_by_binding_energy) then
           ...
           if( getsymmetriePotFlag_baryon() ) then ! With isospin asymmetry:
        ...
        if( g_rho.ne.0. ) then ! With isospin asymmetry:
           MomChooseType = 2 ! = with isospin asymmetry
        else
           MomChooseType = 3 ! = without isospin asymmetry
        endif
