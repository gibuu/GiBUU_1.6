...
!   public :: width_gammaNR
  ...
  ! SOURCE
  !
  real, save :: MA=1.3   ! 0.95 tuned to ANL  ! 1.3 tuned to BNL
  ! PURPOSE
...
contains
  subroutine readInputResProdFormFactors
    ...
    ! * DeltaCouplrelErr
    !*************************************************************************
    NAMELIST /input_FF_ResProd/ FF_ResProd,debugflag,MA,aDelta,bDelta,cDelta,&
         ...
         & W_cutOff_switchAll,DeltaCouplrelErr
    ...
    if (refit_barnu_axialFF .and. HNV_axialFF) then
       stop 'Only of of the parameters HNV_axialFF  _OR_  refit_barnu_axialFF can be true, not both. STOP'
    end if
  ...
  function getFormfactor_Res (Qs, bare_mass, resID, targetCharge, process, FF_set) result (formfactorfield)
    ...
    if(W_cutoff_switch.and.(resID.eq.Delta)) then
       if(bare_mass.gt.0) then
  ...
  subroutine vectorformfactor_MAID_CM(vecformfactor,Qs,resID,targetCharge,process,FF_set)
    ...
    if(firstTime.and.useTable) then
       ...
       write(*,*)
       write(*,*) 'Tabulating form factors for resonance production using CM helicity amplitudes..'
