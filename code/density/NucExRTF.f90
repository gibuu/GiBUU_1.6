...
contains 
  ...
  subroutine NucExRTF_Main(A_nuc,Z_nuc,RTF_dens)
    ...
    integer :: j
    real :: gs_rtf, gv_rtf, gr_rtf, ms_rtf, mv_rtf, mr_rtf
    real :: g2_rtf, g3_rtf
    real :: xn,yn1,yn2,dummy
    real, dimension(1:200)  :: xx,y1,y2
    real, dimension(0:2000) :: x_new

    real, dimension(1:200)  :: y3, y4, y5, y6
    real, dimension(0:2000) :: sigmaPot, omegaPot, rhoPot, coulombPot
    real :: mstar, pf, yn3, yn4, yn5, yn6  ! ,Ef
...
!                                !          & 980.000,0,1,2.49,0   !scalar - isovector
    ...
    do j=0,2000
       xn = float(j)*dx
       call Bsplint2(xx,y1,xn,yn1)
       call Bsplint2(xx,y2,xn,yn2)
       x_new(j)  = xn
       RTF_dens(j,1) = yn1
       RTF_dens(j,2) = yn2
    end do
    ...
    do j=0,2000
       xn = float(j)*dx
       call Bsplint2(xx,y3,xn,yn3)
       call Bsplint2(xx,y4,xn,yn4)
       call Bsplint2(xx,y5,xn,yn5)
       call Bsplint2(xx,y6,xn,yn6)
       x_new(j)      = xn
       sigmaPot(j)   = yn3
       omegaPot(j)   = yn4
       rhoPot(j)     = yn5
