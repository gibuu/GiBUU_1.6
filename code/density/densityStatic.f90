...
contains
  ...
  subroutine TabulateZero(nuc)
    ...
    write(*,paragraph) 'Initializing density tabulation (density=0.)'
  ...
  subroutine TabulateSphere(nuc)
    ...
    write(*,paragraph) 'Initializing density tabulation (Sphere)'
    write(*,'(A,F8.4)') '    dr   =',nuc%dx
    write(*,'(A,F8.4)') '    r_max=',nuc%dx*float(nuc%maxIndex)
    ...
    If(nuc%radius.lt.epsilon) then
       ...
       write(*,*) 'Not possible to use that nucleus for sphere density. STOP'
       stop
    end if
  ...
  subroutine TabulateDensityWoodsSaxon (nuc)
    ...
    write(*,paragraph) 'Initializing density tabulation (Woods-Saxon)'
    write(*,'(A,F8.4)') '    dr   =',nuc%dx
    write(*,'(A,F8.4)') '    r_max=',nuc%dx*float(nuc%maxIndex)
    ...
    If (nuc%surface<epsilon .or. nuc%radius<epsilon) then
       ...
       write(*,*) 'A=',nuc%mass,'  Z= ',nuc%charge
       write(*,*) 'Not possible to use that nucleus for Woods-Saxon density. STOP'
       stop
    end if
  ...
  subroutine TabulateDensityHarmOsc(nuc)
    ...
    write(*,paragraph) 'Initializing density tabulation (Harm. Osc.)'
    write(*,'(A,F8.4)') '    dr   =',nuc%dx
    write(*,'(A,F8.4)') '    r_max=',nuc%dx*float(nuc%maxIndex)
    ...
    if (.not.okay) then
       ...
       write(*,*) 'Radius=',nuc%radius,'   Surface=',nuc%surface
       write(*,*) 'A=',nuc%mass,'  Z= ',nuc%charge
       write(*,*) 'Not possible to use that nucleus for harm. osc. density. STOP'
       stop
    end if
  ...
  subroutine TabulateDensityLenske (nuc)
    ...
    write(*,paragraph) 'Initializing density tabulation (Lenske & Woods-Saxon)'
    write(*,'(A,F8.4)') '    dr   =',nuc%dx
    write(*,'(A,F8.4)') '    r_max=',nuc%dx*float(nuc%maxIndex)
  ...
  subroutine TabulateDensityLuis(nuc)
    ...
    write(*,paragraph) 'Initializing density tabulation (NPA554)'
    write(*,'(A,F8.4)') '    dr   =',nuc%dx
    write(*,'(A,F8.4)') '    r_max=',nuc%dx*float(nuc%maxIndex)
  ...
  subroutine TabulateDensityExRTF(nuc)
    ...
    write(*,paragraph) 'Initializing density tabulation (ExRTF Lenske)'
    write(*,'(A,F8.4)') '    dr   =',nuc%dx
    write(*,'(A,F8.4)') '    r_max=',nuc%dx*float(nuc%maxIndex)
  ...
  subroutine SearchMaxVals(nuc)
    ...
    write(*,*) 'Integrations:'
    write(*,'("     ",A10,3A25)')   ' ', 'int(rho(r) d^3r)', 'int(rho(r) r d^3r)', 'int(rho(r)r^2 d^3r)'
    write(*,'("     ",A10,3f25.3)') 'Proton:  ',Sum(0,1)*4.*pi,Sum(1,1)*4.*pi,Sum(2,1)*4.*pi
    write(*,'("     ",A10,3f25.3)') 'Neutron: ',Sum(0,2)*4.*pi,Sum(1,2)*4.*pi,Sum(2,2)*4.*pi
    write(*,'("     ",A10,3f25.3)') 'Baryon:  ',Sum(0,3)*4.*pi,Sum(1,3)*4.*pi,Sum(2,3)*4.*pi
    write(*,*)
  ...
  type(dichte) function staticDensity(r,nucl)
    ...
    if (nucl%DoInit) then
       write(*,*) 'nucleus not initialized! stop'
       stop
    endif
    ...
    subroutine denspar(z,a,rp,ap,rn,an)
      ...
      case (50) ! Sn-isotopes:
         ...
         case(124)
            ...
            an=0.558
         case default
            write(*,*)' There is no init for this Sn isotope'
            stop
         end select
      case (73) ! Ta(181) 
         ! proton values from DeJager et al
  ...
  ! NAME
  ! subroutine ReAdjust(nuc, potP, potN)
  !
  ! PURPOSE
