...
!
! PURPOSE
! Administrates the calculation of the density of testparticles.
!***************************************************************************
module densitymodule
  ...
  ! * 1: Dynamic Density according to test-particle density
  ! * 2: Static Density (not for Heavy Ions).
  ...
  !*************************************************************************
  !****g* densitymodule/splineExtrapolation
  ! SOURCE
  !
  logical, save :: splineExtrapolation=.true.
  ! PURPOSE
  ! If this switch is 'true', then  dynamic density uses simple spline to 
  ! evaluate density outside the gridN
  !*************************************************************************
...
!!$  end function hadronicPotEnergy
  ...
  subroutine init
    ...
    ! * numberLargePoints
    !***************************************************************************
    NAMELIST /initDensity/ densitySwitch, splineExtrapolation, densityInput_proton, densityInput_neutron, &
    ...
    write(*,'(A,L6,A)') '  Set splineExtrapolation to  ',splineExtrapolation,'.'
  ...
  ! NAME
  ! real function getBaryonDensity (pos) result (baryonDensity)
  ! PURPOSE
  ...
  real function getBaryonDensity (pos)
    ...
    i1 = nint(pos(1)/gridSpacing(1))
    i2 = nint(pos(2)/gridSpacing(2))
    i3 = nint(pos(3)/gridSpacing(3))

    if (abs(i1)<=gridPoints(1) .and. abs(i2) <=gridPoints(2) .and. abs(i3)<=gridPoints(3)) then
      ...
      getBaryonDensity = (densityField(i1,i2,i3)%baryon(0) + factor*totalDensity(i1,i2,i3)) / (1.+factor)
  ...
  ! USAGE
  ! * (dichte)=densityAt(r) with  real(1:3) r
  ! USES
  ! * nucleus
  !*************************************************************************
  type(dichte) function densityAt(r)
    ...
    real,dimension(1:3)    :: rnew      !position where density should be calculated
    ...
    Case(1) !Dynamic Density according to test-particle density

       if (GetGridIndex(r,indizes,0)) then
          ! inside grid
          if (splineExtrapolation) then
             densityAt=extrapolate(r)
          ...
          If(continousBoundaries) then
             ...
             rnew=r
             do i=1,3
                IF(r(i).gt.gridSize(i)) then
                   rNew(i)=r(i)-2.*gridsize(i)
                else iF(r(i).lt.-gridSize(i)) then
                   rNew(i)=r(i)+2.*gridsize(i)
                end if
             end do
  ...
  contains
    ...
    !***********************************************************************
    function extrapolate(r)
      use inputGeneral , only : continousBoundaries

      type(dichte)::  extrapolate
      ...
      extrapolate%baryon=0.
      extrapolate%proton=0.
      extrapolate%neutron=0.
      Do i=0,7
         ...
         If ((Abs(grid(i,1)).le.gridPoints(1)).and.(Abs(grid(i,2)).le.gridPoints(2))&
            ...
            extrapolate=extrapolate+(densityField(grid(i,1),grid(i,2),grid(i,3))*factor)
         end if
      End do
  ...
  subroutine updateDensity(teilchen)
    ...
    type(dichte),save :: densZero ! should be initialized to zero !!!
...
!                       &+ isofact*fact*g_rho*rhoField(I1,I2,I3,1:3)
         momentum(1:3) = teilchen%momentum(1:3) + SelfEnergy_vector(i1,i2,i3,0,teilchen%Id,teilchen%charge,teilchen%antiparticle)
       else
  ...
  ! PURPOSE
  ! return the index of the corresponding coordinate.
