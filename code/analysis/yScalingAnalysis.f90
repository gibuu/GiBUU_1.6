...
!   end subroutine debug_prompt
  ...
  subroutine cc1Xsection(q,p,Eps,m0A,m0A_1,wl,wt)
    ...
    type(tnucleus),pointer :: NucInfo
    integer :: Z,N
    real :: E,En,Eb,kap,tau,taub,eta,del2,GEp,GMp,GEn,GMn,Ge2,Gm2,delG,W2,delW1,delW2, dummy, Qsquared !,W1
    ...
    QSquared = tau * (4.*mN**2)
    call formfactors_QE(QSquared,1,0, dummy, dummy, GE=GEn,GM= GMn)
    call formfactors_QE(QSquared,1,1, dummy, dummy, GE=GEp, GM=GMp)
...
!     W1 = tau*Gm2
  ...
  function scalingFunction(var,Xsection,Li,Lf,Qsquared,theta,process_ID,flavor_ID, GL, GT)
    ...
    real, optional , intent(out) :: GL
    real, optional , intent(out) :: GT
    real :: scalingFunction
    real :: GEp, GMp, GEn, GMn, mott, q, kap, vt, vl, tau, delta, psi, &
          & ksi, W2, wl,wt, y, dummy, ml, sig0 , tanthetaTh2, kp , FA,GA,GAp,FP,GP,GE,GM, &
