...
5                 format(i4,1x,i2,1x,f6.3,3(1x,f8.3),3(1x,f8.3),1x,i5,1x,i4)
  ...
  subroutine DoHeavyIonAnalysisTime (realPV, time, delta_T)
    ...
    use IdTable, only: pion, EOV, NOP

    type(particle),dimension(:,:),intent(in) :: realPV ! particleVectors
    real, intent(in) :: time,delta_T
    integer          :: i,j,k,fact,indFree
      ...
      Loop_over_ensembles: do i = 1,size(realPV,dim=1)
         Loop_over_particles : do j = 1,size(realPV,dim=2)
            ...
            if(realPV(i,j)%ID > 0) then
               ...
               !effective mass (RMF --> Dirac / Skyrme --> non-rel. definition):
               p2=dot_product(realPV(i,j)%momentum(1:3),realPV(i,j)%momentum(1:3))
               emass = realPV(i,j)%momentum(0)**2 - p2
               ...
               write(103,50) fact*realPV(i,j)%ID,realPV(i,j)%charge,&
                    realPV(i,j)%mass,emass,&
                    indFree,&
                    (realPV(i,j)%position(k), k=1,3),&
                    (realPV(i,j)%momentum(k), k=1,3),i,isut+1,stossParameter
            endif
         enddo Loop_over_particles
      enddo Loop_over_ensembles
      close(103)

50    format(i4,1x,i2,1x,2(f8.4,1x),1x,i1,3(1x,f8.3),3(1x,f8.3),1x,i5,1x,i4,1x,f5.2)
    ...
    subroutine RapiditySpectra
      ...
      integer, parameter :: nrap = 61
      real, save :: ystart=-6.0,dy=0.1,Norm
      real, dimension(-nrap:nrap), save :: yBin

      real, dimension(-nrap:nrap) :: dndy,dndy2
      integer :: ibin,ibin2
      real :: ystep,yb,p0,pz
      real :: vz,yb2
      ...
      if (RapidityInitFLAG) then
         Norm = 1./ ( float( size(realPV,dim=1) ) * dy )
         ystep = ystart
         do i=-nrap,nrap
            ybin(i) = ystep
            ystep = ystep + dy
      ...
      Loop_over_ensembles: do i = 1,size(realPV,dim=1)
         Loop_over_particles : do j = 1,size(realPV,dim=2)
            ...
            if (realPV(i,j)%ID == 1) then
               ...
               if (abs(ibin) > nrap .or. abs(ibin2) > nrap) then
                  write(*,*) yb,ibin,ibin2
                  STOP
               endif

               dndy(ibin) = dndy(ibin) + Norm
               dndy2(ibin2) = dndy2(ibin2) + Norm
...
50    format(20e12.4)
      ...
      do i=-nrap,nrap
         write(203,50) yBin(i),dndy(i),dndy2(i)
      end do
  ...
  subroutine HeavyIon_evol (realparticles, time)
    ...
    real :: rhoz_n, rhoz_p, Ef_p, Ef_n, pf_p, pf_n
    ...
    if (icall==0) then
       ...
       write(33,'(A)')'# time rhocen_baryons rhocen_antibaryons ptot baryon_number charge strangeness temperature(central)'
    ...
    write(32,5) time,sum(parnum(nucleon,:)),sum(parnum(delta,:)),&
         ...
         sum(parnum(sigmaStar:sigma_1915,:)),&
         sum(parnum(-Lambda,:)),sum(parnum(-SigmaResonance,:)),&
         sum(parnum(-sigma_1915:-sigmaStar,:)),&
...
5   format(50(1x,e13.6))
    ...
    write(33,5) time, rhoz_bar, rhoz_antibar, p(:), baryon_number, charge_number, strangeness, temperatureAt((/0.,0.,0./))
    ...
    write(44,5) time, float(CountedEvents(0,2,1))/float(numensembles), float(CountedEvents(1,2,1))/float(numensembles)
