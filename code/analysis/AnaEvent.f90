...
!
! The routines given in this module decay into two categories.
...
module AnaEvent
  ...
  use IDTABLE, only : pion, eta,kaon,kaonBar,DMeson, dBar, &
...
!   end function event_getMultiplicities
  ...
  subroutine event_sigma(E,sigmaOut,newInit,runNumber,identifier)
    ...
    character(30), dimension(1:dimSigma,1:2) :: sigma_name  ! Denotes the different channels, used for output
    ...
    idLoop: do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
       chargeLoop: do charge=-2,2
          ...
          eventLoop: do j=lBound(E,dim=1),uBound(E,dim=1)
             ! (2) Count contributions of any particles in the event with charge="charge" and ID="particleIDs(i):
    ...
    do numberParticles=1,4
       idLoop1: do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
          ...
          sigma_name(channel,2)=intTochar(channel+dimSigma)//': error '//intTochar(numberParticles)//trim(name)
          ...
          eventLoop1: do j=lBound(E,dim=1),uBound(E,dim=1)
             ...
             ! Count contributions of any particles in the event with any charge="charge" and ID="particleIDs(i)":
             ch_loop1: do charge=-2,2
                ...
                numInEventLoop: do num=1,numberParticles
                   ...
                   if( .not.event_GetParticle(E(j),particleIDs(i),charge,num,part)) exit numInEventLoop
    ...
    eventLoop2: do j=lBound(E,dim=1),uBound(E,dim=1)
       if(( Sum(E(j)%numberParticles(1,:)).eq.1).and.( Sum(E(j)%numberParticles(7,:)).eq.1)) then
          foundIDs=0
          ! Search pion
...
!          if(E(j)%numberParticles(7,0).ne.0) cycle
    ...
    do j=lBound(E,dim=1),uBound(E,dim=1)
       if(sum(E(j)%numberParticles(1,-1:1)).eq.0) then
          perweight=0.
          if(event_GetParticle(E(j),outLeptonID,outLeptonCharge,1,part))  perweight(1)=part%perweight
          !if(associated(E(j)%particleList%first))  perweight(1)=E(j)%particleList%first%V%perweight
    ...
    ! - no sum over the different charge states
    ! - If there is another particle of the same species, but with another charge then we count the event.
    ! - E.g. for pi^+ pi^0 the event contributes both to "pi+ + X" and "pi0 + X"
    !*************************************************************
    idLoop_single: Do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
       chargeLoop_single: do charge=-2,2
          if(validCharge_ID(particleIDs(i),charge)) then          ! Check that channel is valid
             channel=channel+1
    ...
    ! - count each pi^0 seperately, e.g. if there are two they count twice!
    !*************************************************************
    ...
    eventLoop_Krusche3: do j=lBound(E,dim=1),uBound(E,dim=1)
       ...
       if( E(j)%numberParticles(7,1).ge.1) then
          if( event_GetParticle(E(j),particleIDs(7),1,1,part)) then
             sigma(channel,1)=sigma(channel,1)+part%perweight* E(j)%numberParticles(7,1)
          else
             write(*,*) 'Error in event_sigma: Krusche p', E(j)%numberParticles(7,1), 'Stop!'
             stop
          end if
       end if
    ...
    eventLoop_Krusche4: do j=lBound(E,dim=1),uBound(E,dim=1)
       ! (2) Search for p and count each p seperately:
       if( E(j)%numberParticles(7,0).ge.1) then
          if( event_GetParticle(E(j),particleIDs(7),0,1,part)) then
             sigma(channel,1)=sigma(channel,1)+part%perweight* E(j)%numberParticles(7,0)
          else
             write(*,*) 'Error in event_sigma: Krusche n', E(j)%numberParticles(7,0), 'Stop!'
             stop
          end if
       end if
    ...
    do j=lBound(E,dim=1),uBound(E,dim=1)
       if(sum(E(j)%numberParticles(7,0:1)).eq.0) then
          perweight=0.
          if(event_GetParticle(E(j),outLeptonID,outLeptonCharge,1,part))  perweight(1)=part%perweight
          sigma(channel,1)=sigma(channel,1)+perweight(1)
       end if
    ...
    do j=lBound(E,dim=1),uBound(E,dim=1)
       if(sum(E(j)%numberParticles(7,0:1)).ge.5) then
          perweight=0.
...
!!$       end do
       ...
       do i=1,100
          if(runNumber.gt.1) then
             ...
             sigma(i,2)=sqrt(max(((sigma(i,2)-sigma(i,1)**2/float(runNumber))/(float((runNumber-1)*(runNumber)))),0.))
          else
             sigma(i,2)=0.
          end if
       end do
  ...
  subroutine event_dSigma_dOmega(E,dTheta,dPhi,string,runNumber,hists_theta,hists_phi,hists2D,initHists,makeOutputIn,sameFileNameIn)
    ...
    ! Optional
    type(histogram)  ,optional, intent(inout),dimension(1:numStableParts,-2:2) :: hists_theta,hists_phi
    ...
    if(present(hists_theta).neqv.present(hists_phi).or.present(hists2D).neqv.present(initHists).or. &
         & present(hists_phi).neqv.present(hists2D)) then
       write(*,*) 'Problem in AnaEvent/event_dSigma_dOmega:'
       write(*,*) 'You have to give either no or all optional arguments!! Stop'
       stop
    end if
    if(present(hists_theta).and.present(hists_phi).and.present(hists2D).and.present(initHists)) then
       optionals=.true.
    else
       optionals=.false.
    end if
    ...
    idLoop: do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
       ...
       chargeLoop: do charge=-2,2
          ...
          ! Count contributions of any particles in the event with charge="charge" and ID="particleIDs(i):
          eventLoop: do j=lBound(E,dim=1),uBound(E,dim=1)
             if((E(j)%numberParticles(i,charge).eq.1).and.( sum(E(j)%numberParticles(i,:)).eq.1)) then
                if( event_GetParticle(E(j),particleIDs(i),charge,1,part)) then
                   ...
                   write(*,*) 'Error in event_dSigma_dOmega', E(j)%numberParticles(i,charge), 'Stop!'
                   stop
                end if
             end if
          end do eventLoop
          ...
          ! Output histograms
          If(optionals) then
             ! Make error calculation, therefore subtract the input from the present histogram to get the change
  ...
  subroutine event_dSigma_dE(E,eMin,eMax,dE,string,runNumber,hists,initHists,makeOutputIn,sameFileNameIn,histsMulti,hists1X,hists2X)
    ...
    character(80) :: filename,filename1X,filename2X,name,filenameMulti
    real :: ekin
    ...
    idLoop: Do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
       ...
       chargeLoop: do charge=-2,2
          ...
          name = PartName(particleIds(i),charge,.false.)
          if(makeInitHists) then
             call createHist(dsigma_dE, 'dSigma/dEkin('// trim(name) //&
                  & ') for single '//trim(name)// ' production' &
                  &  ,eMin ,eMax,dE)
         ...
         name = PartName(particleIDs(i))
          if(sameFileName) then
             filename=trim(string)//'_dSigma_dEkin_'// trim(name)//'_charge_'//intTochar_pm(charge) //'.dat'
             filename1X=trim(string)//'_dSigma_dEkin_'// trim(name)//'_charge_'//intTochar_pm(charge) //'_1X.dat'
             filename2X=trim(string)//'_dSigma_dEkin_'// trim(name)//'_charge_'//intTochar_pm(charge) //'_2X.dat'
             filenameMulti=trim(string)//'_dSigma_dEkin_'// trim(name)//'_charge_'//intTochar_pm(charge) //'_MULTI.dat'
          else
          ...
          ! Loop for single-particle cross sections (e.g. 1pi+ and no other pions in the event)
          ...
          eventLoop: do j=lBound(E,dim=1),uBound(E,dim=1)
             if(( E(j)%numberParticles(i,charge).eq.1).and.( sum(E(j)%numberParticles(i,:)).eq.1)) then
                if( event_GetParticle(E(j),particleIDs(i),charge,1,part)) then
                   ...
                   write(*,*) 'Error in event_dSigma_dE', E(j)%numberParticles(i,charge), 'Stop!'
                   stop
                end if
             end if
             ...
             ! Loop for 1-particle-plus-X cross sections (e.g. 1pi+ and other pi0, pi-  in the event)
             !Find out whether there are any particles in the event with charge="charge" and ID="particleIDs(i):
             if(( E(j)%numberParticles(i,charge).eq.1)) then
                if( event_GetParticle(E(j),particleIDs(i),charge,1,part)) then
                   ...
                   write(*,*) 'Error in event_dSigma_dE_1X', E(j)%numberParticles(i,charge), 'Stop!'
                   stop
                end if
             end if
             ...
             ! Loop for 2-particle-plus-X cross sections (e.g. 2pi+ and other pi0, pi-  in the event)
             !Find out whether there are any particles in the event with charge="charge" and ID="particleIDs(i):
             if(E(j)%numberParticles(i,charge).eq.2) then
                do particle_number=1,2
                   if( event_GetParticle(E(j),particleIDs(i),charge,particle_number,part)) then
                      ...
                      write(*,*) 'Error in event_dSigma_dE_2X', E(j)%numberParticles(i,charge), 'Stop!'
                      stop
                   end if
                end do
             end if
             ...
             !Find out whether there are any particles in the event with charge="charge" and ID="particleIDs(i):
             if(E(j)%numberParticles(i,charge).ge.1) then
                !write(*,'(3(A,I5))') 'i=',i, '   charge=',charge,  'numberParticles=',E(j)%numberParticles(i,charge)
                do particle_number=1,E(j)%numberParticles(i,charge)
                   if( event_GetParticle(E(j),particleIDs(i),charge,particle_number,part)) then
                      ekin=part%momentum(0)-part%mass
                      !write(*,'(A,I5,2(A,g15.5))') 'particle_number=', particle_number, '   Ekin=',ekin, '    perweight=',part%perweight
                      call addHist(dsigma_dE_multi , ekin  ,part%perweight)
                   else
                      write(*,*) 'Error in event_dSigma_dE_multi', E(j)%numberParticles(i,charge), 'Stop!'
                      stop
                   end if
                end do
             end if
  ...
  !   and "STRING" by the input string.
  !*************************************************************************
  subroutine event_dSigma_dEcostheta(E,EcosthetaMin,EcosthetaMax,dEcostheta,string,runNumber,hists,hists_c, &
               & hists_MULTI, hists_c_MULTI,  initHists,  makeOutputIn, sameFileNameIn)
    use particleDefinition
    use IdTable, only: isHadron
    ...
    ! Local
    type(particle) :: part
    type(histogram) ::    dsigma_dEcostheta, dsigma_dcostheta, dsigma_dEcostheta_MULTI, dsigma_dcostheta_MULTI
    integer :: charge,i,j, particle_number
    character(80) :: name, filename_Ecostheta, filename_costheta, filename_Ecostheta_MULTI, filename_costheta_MULTI
    logical :: makeInitHists
    ...
    idLoop: Do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
       ...
       chargeLoop: do charge=-2,2
          if(validCharge_ID(particleIDs(i),charge)) then
             ...
             if(makeInitHists) then
             ! Initialize histogram
                call createHist(dsigma_dEcostheta, 'dSigma/dEcostheta('// trim(name) //&
                     & ') for single '//trim(name)// ' production' &
                     &  ,EcosthetaMin,EcosthetaMax,dEcostheta)
                call createHist(dsigma_dcostheta, 'dSigma/dcostheta('// trim(name) //&
                     & ') for single '//trim(name)// ' production' &
                     &  ,-1.0,1.0,0.02)
             ...
             if(sameFileName) then
                filename_Ecostheta=trim(string)//'_dSigma_dEcostheta_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'.dat'
                filename_costheta=trim(string)//'_dSigma_dcostheta_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'.dat'
             ...
             if((E(j)%numberParticles(i,charge).eq.1).and.( sum(E(j)%numberParticles(i,:)).eq.1)) then
                   if( event_GetParticle(E(j),particleIDs(i),charge,1,part)) then
                      If(dot_product(part%momentum(1:3),part%momentum(1:3)).gt.0) then
                         costheta=part%momentum(3)/sqrt(dot_product(part%momentum(1:3),part%momentum(1:3)))
                      else
                         costheta=0.
                      end if
                      ...
                      write(*,*) 'Error in event_dSigma_dEcostheta', E(j)%numberParticles(i,charge), 'Stop!'
                      stop
                   end if
                end if
             ...
             if(E(j)%numberParticles(i,charge).ge.1) then
               do particle_number=1,E(j)%numberParticles(i,charge)
                   if( event_GetParticle(E(j),particleIDs(i),charge,particle_number,part)) then
                      ...
                      write(*,*) 'Error in event_dSigma_dEcostheta_MULTI', E(j)%numberParticles(i,charge), 'Stop!'
                      stop
                   end if
                end do
             end if
  ...
  !*************************************************************************
  subroutine event_dSigma_dInvMass(E,string,runNumber, &
       & dW_Npi,Wmin_Npi,Wmax_Npi,histsW_nucleon_pion, dW_mupi,Wmin_mupi,Wmax_mupi,histsW_muon_pion, &
       & dW_muN,Wmin_muN,Wmax_muN,histsW_muon_nucleon, initHists,makeOutputIn,sameFileNameIn)
    ...
    character(*), intent(in) :: string ! used as prefix for all output files
    integer, intent(in) :: runNumber ! number of the run
    type(histogram),optional, intent(inout),dimension(-1:1) :: histsW_nucleon_pion, histsW_muon_pion, histsW_muon_nucleon
    real, intent(in)    :: dW_Npi,Wmin_Npi,Wmax_Npi,   dW_mupi,Wmin_mupi,Wmax_mupi,   dW_muN,Wmin_muN,Wmax_muN
    logical, intent(in),optional :: initHists
    logical , intent(in), optional ::makeOutputIn ! Flag to switch off output : .false. = no output
    logical , intent(in), optional ::sameFileNameIn ! .true. = always print to same filenames., .false. = different files for different runs
    ...
    pionChargeLoop: do charge=-1,1
       ...
       if(makeInitHists) then
          ...
          !          write(*,'(9(A,g10.3))') 'Wmin_Npi=',Wmin_Npi, '   Wmax_Npi=',Wmax_Npi,  '  dW_Npi=',dW_Npi, &
          !                             & '   Wmin_Npi=',Wmin_mupi, '   Wmax_Npi=',Wmax_mupi,  '  dW_Npi=',dW_mupi, &
          !                             & '   Wmin_Npi=',Wmin_muN, '   Wmax_Npi=',Wmax_muN,  '  dW_Npi=',dW_muN

       else
          dsigma_dW=histsW_nucleon_pion(charge)
          dsigma_dW_muon_pion=histsW_muon_pion(charge)
          dsigma_dW_muon_nucleon=histsW_muon_nucleon(charge)  ! this is the charge of pion, not nucleon
       end if
       ...
       eventLoop: do j=lBound(E,dim=1),uBound(E,dim=1)
          ...
          if((          E(j)%numberParticles(1,charge).eq.1) .and. &  ! in numberParticles pion=1
               ...
               & sum(E(j)%numberParticles(7,:)).eq.1       ) then ! in numberParticles nucleon=7
             ...
             if( event_GetParticle(E(j),101,charge,1,part_pion)) then  ! ID of pion is 101
                ...
                do k=0,1
                   if(  event_GetParticle(E(j),1,k,1,part_nucleon) ) then ! ID of nucleon is 1
                      nucleon_charge=k
                      !write(*,'(A,4g12.5,A,g12.5)') 'Found nucleon with 4-momentum', part_nucleon%momentum, &
                ...
                if(nucleon_charge.ge.0 .and. event_GetParticle(E(j),outLeptonID,outLeptonCharge,1,part_muon) ) then

                   !write(*,'(A,4g12.5,A,g12.5)') 'Found lepton with 4-momentum', part_muon%momentum, &
                   ...
                   write(*,*) 'Error in event_dSigma_dInvMass. Cannot get muon from the event. Stop!'
                   stop
                end if
             ...
             if (W2.gt.0 .and. W2_muon_pion.gt.0 .and. W2_muon_nucleon.gt.0) then
                ...
                write(*,'(4(A,g12.5))') 'Error in event_dSigma_dInvMAss. One of invariant masses is negative W2=', &
       ...
       if(present(histsW_nucleon_pion) .and. present(histsW_muon_pion) .and. present(histsW_muon_nucleon))then
          if(.not.makeInitHists) then
             ! Calculate the square of the changes compared to last run:
  ...
  ! In hist_cos you will find the dsigma_dcostheta(angle of the outgoing lepton relative to incoming neutrino).
  ...
  !   and "STRING" by the input string.
  !*************************************************************************
  subroutine event_dSigma_dLeptonVariables(E,eMin,eMax,dE,string,runNumber,hist_Enu,hist_E,hist_cos,hist_Q2,&
       & initHists, makeOutputIn, sameFileNameIn, specificEvent)
    ...
    if(makeInitHists) then
       call createHist(dsigma_dEnu, 'dSigma/dEnu( incoming neutrino ) for events'//trim(name) ,eMin ,eMax,dE)
       call createHist(dsigma_dE,   'dSigma/dEkin( outgoing lepton ) for events'//trim(name) ,eMin ,eMax,dE)
       call createHist(dsigma_dcos, 'dSigma/dcostheta( outgoing  lepton ) for events'//trim(name), -1.0, 1.0, 0.02)
       ! Q2bin and Q2max are taken numerically the same as those for energy
       call createHist(dsigma_dQ2,  'dSigma/dQ^2( outgoing  lepton ) for events'//trim(name), 0.0, eMax, dE)
    else
    ...
    if(sameFileName) then
       ...
       filename_Enu=trim(string)//'_dSigma_dEnu_lepton_'//trim(name)//'.'//trim(intTochar(runNumber))//'.dat'
       filename_E=trim(string)//'_dSigma_dEkin_lepton_'//trim(name)//'.'//trim(intTochar(runNumber))//'.dat'
       filename_cos=trim(string)//'_dSigma_dcos_lepton_'//trim(name)//'.'//trim(intTochar(runNumber))//'.dat'
       filename_Q2=trim(string)//'_dSigma_dQ2_lepton_'//trim(name)//'.'//trim(intTochar(runNumber))//'.dat'
    end if
    ...
    eventLoop: do j=lBound(E,dim=1),uBound(E,dim=1)
       ...
       if (pass) then
          ...
          If(dot_product(lep_mom(1:3),lep_mom(1:3)).gt.0) then
             ...
             write(*,*) 'In event_dSigma_dLeptonVariables strange |k2_prime|=', dot_product(lep_mom(1:3),lep_mom(1:3))
             costheta=-2.
          end if
...
!   end subroutine event_Print
  ...
  ! NAME
  ! subroutine event_dump(L,filename,onlyFreeParticles)
  !
  ! PURPOSE
  ...
  subroutine event_dump(run,L,fileName,onlyFreeParticles,writeNeutrinoProdID)
    ...
    if (run>1 .and. ex) then
      ...
      write(47,outA) '1:Run','2:Event','3:ID','4:Charge','5:perweight','6:position(1)','7:position(2)','8:position(3)',&
                     & '9:momentum(0)', '10:momentum(1)','11:momentum(2)','12:momentum(3)','13:history', '14:production_ID', &
                     & '15:enu'
    end if
    ...
    eventLoop:  do i=lbound(L,dim=1),ubound(L,dim=1)
       ...
       do
          ...
          if(onlyFree.and.pNode%V%ID.eq.nucleon) then
             ! Check that nucleon may become free, that means that it has in vacuum a well-defined momentum,
             ! i.e. p(0)^2-m_n^2 > 0
          ...
          if(NeutrinoProdID) then
             ...
             if(printParticle) write(47,out) run,i,pNode%V%ID,pNode%V%Charge,pNode%V%perweight,pNode%V%position,pNode%V%momentum &
                  & ,pNode%V%history,prod_id, enu
          else
             if(printParticle) write(47,outShort) run,i,pNode%V%ID,pNode%V%Charge,pNode%V%perweight,pNode%V%position &
  ...
  subroutine event_add(L, V)
    ...
    if(.not.V%antiparticle) then
       ...
       case Default
          L%numberParticles(12,V%charge)=L%numberParticles(12,V%charge)+1
       end select
    ...
    if(numRuns.le.1) then
       ...
       do i=lbound(a%yval,dim=1),ubound(a%yval,dim=1)
          ...
          if(a%yval(i,3).lt.(a%yval(i,1)**2)/float(numRuns)) then
             write(*,'(A,2E20.10)') 'Error in setError_hist',a%yval(i,3),(a%yval(i,1)**2)/float(numRuns)
             ! If there is an error check whether it is due to rounding errors:
             if(abs(a%yval(i,3)).gt.epsilon) then
                if(abs(a%yval(i,3)-(a%yval(i,1)**2)/float(numRuns))/abs(a%yval(i,3)).gt.epsilon) then
                   call traceback()
                   stop 'setError_hist'
                end if
             end if
             ...
             dummy=float(numRuns)*sqrt(max(1./float(numRuns-1)/float(numRuns)*(a%yval(i,3)-(a%yval(i,1)**2)/float(numRuns)),0.))
             a%yval(i,3)=dummy
          end if
       end do
    end if
...
!   end subroutine event_DumpNumbers
  ...
  subroutine event_pairPhotons(L,fileName,dsigma_dm_threeGammas,Hist_massPhotons)
    ...
    character(100) :: fileName_data,fileName_statistics,filename_dsdm    ! The file name to write to
    type(tParticleListNode),Pointer       :: pNode
    ...
    if (.not.allocated(Hist_massPhotons%yVal)) then
       call CreateHist2D(Hist_massPhotons, 'Mass(2 gamma) vs. Mass(3 gamma)',(/0.,0./),(/1.0,1.0/),(/0.03,0.03/),.true.)
    ...
    eventLoop:  do i=lbound(L,dim=1),ubound(L,dim=1)
       ...
       if(atLeastOneParticle) then
          ...
          if(numPhotons.ne.0) then
             list_numPhotons(numPhotons,2) =list_numPhotons(numPhotons,2)+minval(perweight(1:numPhotons))
          else
          ...
          do massLoop_i=1,numPhotons-2
             do massLoop_j=massLoop_i+1,numPhotons-1
                do massLoop_k=massLoop_j+1,numPhotons
                   !write(*,*) massLoop_i,massLoop_j,massLoop_k,perweight((/massLoop_i, massLoop_j, massLoop_k/))
                   ! Evaluate the mass of the triple and the masses of all possible pairs:
                   call getMasses( momentum((/massLoop_i, massLoop_j, massLoop_k/),0:3),mass_all,mass_pairs)
                   ! Protocol result
                   call AddHist(dsigma_dm_threegammas(0),mass_all,minval(perweight((/massLoop_i, massLoop_j, massLoop_k/)))/numRuns)
                   IF(numPhotons.le.ubound(dsigma_dm_threegammas,dim=1)) then
                      call AddHist(dsigma_dm_threegammas(numPhotons),mass_all,minval(perweight((/massLoop_i, massLoop_j,&
                           &  massLoop_k/)))/numRuns)
                   end if
                   do index=1,3
                      call AddHist2D(hist_massPhotons, (/mass_all,mass_pairs(index) /),&
                           & minval(perweight((/massLoop_i, massLoop_j, massLoop_k/)))/numRuns)
                      if(abs(minval(perweight((/massLoop_i, massLoop_j, massLoop_k/)))).lt.0.0001) then
  ...
  !
  ! Additionally info "prod_id  Enu  Enu_restored  nu  Ehad  perweight" for all events is written in the file
  !
  ...
  subroutine event_hadronicEnergy(L, numin, numax, nubin, Ehad_versusNu, dSigdNu, dSigdEhad, &
    ...
    type(tAnaEvent),dimension(:), intent(in) :: L
    real, intent(in) :: numin, numax, nubin ! boundaries for the hystograms ! in the namelist "nl_calorimetric_analysis" in neutrinoAnalysis.f90
    real, intent(in) :: Enumin, Enumax, Enubin ! boundaries for the hystograms ! in the namelist "nl_calorimetric_analysis"
    type(histogram2D),dimension(0:max_Hist), intent(inout) :: Ehad_versusNu, Enurestored_versusEnu
    type(histogram), dimension(0:max_Hist),intent(inout) :: dSigdNu, dSigdEhad, dSigdEnu, dSigdEnurestored
    character(100) :: filename_Ehad_versusNu, filename_dSigdNu, filename_dSigdEhad, &
                    & filename_Enurestored_versusEnu, filename_dSigdEnu, filename_dSigdEnurestored ! file names to write to
    type(tParticleListNode),Pointer       :: pNode
    integer :: prod_id
    real    :: Ehad, perweight
    real, dimension(0:3) :: lepIn_mom, lep_mom, boson_mom, nuc_mom
    ...
    if (.not.allocated(Ehad_versusNu(0)%yVal)) then
       do iHist=0, max_Hist ! 0=all 1=QE 2=Delta 3=highRES 4=1pibgr 5=DIS 6=2p2hQE 7=2p2hDelta 8=2pibgr
          if (.not.includeHist(iHist)) cycle
          call CreateHist2D(Ehad_versusNu(iHist), 'Nu -- Ehad plane',(/numin,numin/),(/numax,(numax+1.)/),(/nubin,nubin/),.true.)
    ...
    eventLoop:  do i=lbound(L,dim=1),ubound(L,dim=1) ! "i" - first event
       ...
       if (debug) write(*,'(2(A,I5),A,g14.5)') 'first_event=',i, '      prod_ID=',prod_ID,  '     nu=', boson_mom(0)
       Ehad=0.
       ...
       particleLoop: do
          ...
          if( isBaryon(pNode%V%ID) ) then
             ...
             if (pNode%V%antiparticle) then
                Ehad=Ehad+pNode%V%momentum(0)+mN !antibaryon energy + compensation for an accompaning baryon
             else
       ...
       if (debug) write(*,*) ' ------------------------------------ '
       write(60,'(I5,5g14.5)') prod_id, boson_mom(0)+lep_mom(0), Ehad+lep_mom(0), boson_mom(0), Ehad, perweight
  ...
  logical function IfPass_SpecificEvent(specificEvent,event)
    ...
    select case (specificEvent)
    case (1) ! 0 pions
       if ( sum(event%numberParticles(1,:)).eq.0)  IfPass_SpecificEvent=.true.
    case (2) ! 1 proton, X neutrons, no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and. event%numberParticles(7,1).eq.1) &
            & IfPass_SpecificEvent=.true.
    case (3)  ! 1 pi+, no other pions
       if ( sum(event%numberParticles(1,:)).eq.1 .and. event%numberParticles(1,1).eq.1) &
            & IfPass_SpecificEvent=.true.
    case (4)  ! 1 pi0, no other pions
       if ( sum(event%numberParticles(1,:)).eq.1 .and. event%numberParticles(1,0).eq.1) &
            & IfPass_SpecificEvent=.true.
    case (5) ! 1 or more pi0, X other pions
    ...
    case (7) ! 2 protons, no neutrons, no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and. &
                & event%numberParticles(7,1).eq.2 .and. event%numberParticles(7,0).eq.0) &
            & IfPass_SpecificEvent=.true.
    case (8) ! 1 proton 1 neutron, no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and. &
                & event%numberParticles(7,1).eq.1 .and. event%numberParticles(7,0).eq.1) &
            & IfPass_SpecificEvent=.true.
    case (9) ! no protons, 2 neutrons, no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and. &
                & event%numberParticles(7,0).eq.2 .and. event%numberParticles(7,1).eq.0) &
            & IfPass_SpecificEvent=.true.
    case (10) ! 2 protons, any number of neutrons, no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and.  event%numberParticles(7,1).eq.2 ) &
            & IfPass_SpecificEvent=.true.
    case (11) ! 2 neutrons, any number of protons, no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and.  event%numberParticles(7,0).eq.2 ) &
            & IfPass_SpecificEvent=.true.
    case (12) ! 3 protons, X neutrons,  no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and. event%numberParticles(7,1).eq.3 ) &
            & IfPass_SpecificEvent=.true.
    case (13) ! 4 protons, X neutrons,  no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and. event%numberParticles(7,1).eq.4 ) &
            & IfPass_SpecificEvent=.true.
    case (14) ! 1 proton, 0 neutrons, no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and. &
                & event%numberParticles(7,0).eq.0  .and.  event%numberParticles(7,1).eq.1 ) &
            & IfPass_SpecificEvent=.true.
    case (15) ! 0 protons, 1 neutron, no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and. &
                & event%numberParticles(7,1).eq.0 .and.  event%numberParticles(7,0).eq.1)  &
            & IfPass_SpecificEvent=.true.
    case (16) ! 0 protons, X neutrons, no pions
       if ( sum(event%numberParticles(1,:)).eq.0 .and. event%numberParticles(7,1).eq.0) &
            & IfPass_SpecificEvent=.true.
    case default
