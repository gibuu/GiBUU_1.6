...
for (int i=0; i<=numOrigins; i++){
    sprintf(name,"0pions_%dprotons_vs_Enu_"+HistToName[i],outP);
    sprintf(title,"0pions %dprotons: events vs E_{#nu} "+HistToName[i],outP);
    events0pions_vs_EnuTrue[outP][i] = new TH1F (name,title,kk_Enu, Enu_min, Enu_max); }
}
TH1F * events0pions_Allp_vs_EnuTrue[numOrigins+1];
for (int i=0; i<=numOrigins; i++){
    sprintf(name,"0pions_Xprotons_vs_Enu_"+HistToName[i]);
    sprintf(title,"0pions Xprotons: events vs E_{#nu} "+HistToName[i]);
