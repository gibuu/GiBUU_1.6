...
contains
  ...
  subroutine test_Delta
    ...
    use baryonWidthVacuum, only: markusPostFormfactor
    ...
    msqr = 0.
    do i=1,200
      call NDeltaTL (msqr, 1.23, FF(1))
      call NDeltaTL (msqr, 1.43, FF(2))
      call NDeltaTL (msqr, 1.63, FF(3))
      call NDeltaTL (msqr, 1.83, FF(4))
      call NDeltaTL (msqr, 2.03, FF(5))
    ...
    do i=1,100
      W = mN+mPi+i*0.01
      call NDeltaTL (msqr, W, FF(1))
      write (914,'(4G12.5)') W, Delta_FF_Iachello(W,msqr), FF(1), markusPostFormfactor (W, hadron(Delta)%mass, (mN+mPi)**2, 1.071)
    end do
    close(914)
  end subroutine
