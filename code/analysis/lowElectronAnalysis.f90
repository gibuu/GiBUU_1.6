...
contains
  ...
  subroutine lowElectron_Analyze  (particles,finalFlag)
    ...
    use lowElectron, only : le_get_FirstEventRange,le_get_Energy_li,le_get_Energy_lf,le_get_theta_lf
    ...
    real , dimension(1:100,1:2),save :: sigma,sigma_res,sigma_bg,check
    real , dimension(1:100,1:2),save :: sigma_0pions,sigma_0pions_res,sigma_0pions_bg
...
!     if(numberOfCalls.eq.1) then we initialize the histograms
       if(dE_switch) call event_dSigma_dE(events,0.,ekinMax,dEkin,'diff_'//trim(intToChar(numberofFinals)),numberOfCalls &
       ...
       if(dOmega_switch) call event_dSigma_dOmega(events,dTheta,dPhi,'diff_'//trim(intToChar(numberofFinals)) &
            &, numberOfCalls ,dTheta_hists, dPhi_hists, dOmega_hists,(numberOfCalls.eq.1),sameFileNameIn=.true.)


       ! sumed over all energies of the incoming lepton
       if (dE_switch) call event_dSigma_dE(events,0.,ekinMax,dEkin,'diff_ALL', numberOfCallsFinals &
