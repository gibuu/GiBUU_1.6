...
! module check
! PURPOSE
! Here some routines are collected, which e.g. calculated the binding energy
! from the particle vector
!***************************************************************************
module checks
  ...
  PUBLIC :: checkRadius   !, checkRadius_getAmplitude, checkRadius_resetMinMax
!  PUBLIC :: occupied
  PUBLIC :: evaluateTimeStep
  PUBLIC :: CheckGridSize
!  PUBLIC :: evaluateEnergy
!  PUBLIC :: evaluateBindingEnergy_theiss
  PUBLIC :: evaluateTotal4Momentum_RMF
!  public :: checkDensity, checkCoulomb
!  public :: evaluateFermiSurface
!  PUBLIC :: checkEnergyLDA, checkEnergyLDAWelke
!  public :: checkMomentumDensity
!  PUBLIC :: particleTrack, particleTrack_byNumber
  PUBLIC :: ChecksSetDefaulSwitches
!  PUBLIC :: ChecksSwitchOffFrozen
  PUBLIC :: ChecksSwitchRealRun
!   PUBLIC :: ChecksSwitchGS
  PUBLIC :: ChecksCallAll
!   PUBLIC :: ChecksCallOccupied
  ...
  ! PURPOSE
  ! Flag to indicate, whether the momentum density check routine should be 
  ! called
  !*************************************************************************
  ...
  ! PURPOSE
  ! Flag to indicate, whether the occupation chek routine for the real particle
  ...
  ! PURPOSE
  ! Flag to indicate, whether the occupation chek routine for the perturbative 
  ! particle vector should be called
  !*************************************************************************

  !*************************************************************************
  !****g* checks/Do_CallEnergy
  ! SOURCE
  ...
  ! PURPOSE
  ! select, whether the occurence of a 'tachyon' in the check routines
  ! will stop the code or not (Error messages will hopefully occur later
  ! in the code)
  !*************************************************************************
...
contains 
  ...
  ! PURPOSE
  ! Set the default values of the switches "Do_Check???" according the
  ! given eventtype
  ! NOTES
...
!   end subroutine ChecksSwitchGS
  ...
  subroutine Init
    ...
    NAMELIST /checks/ Do_CheckDensity, Do_CheckCoulomb, Do_CheckFermiSurface, &
                      ...
                      Do_OccupiedReal, Do_OccupiedPert, Do_CallEnergy, &
    ...
    write(*,'(A,L1)') '  Do_CallEnergy           = ',Do_CallEnergy
...
!   end function saveEpot
  ...
  !*************************************************************************
  !****s* checks/evaluateBindingEnergy_theiss
  ! NAME
  ...
  subroutine evaluateBindingEnergy_theiss(teilchen,time)
    ...
    if(firstTime) then
       ...
       write(22,*) '# 4: total bound nucleon energy / number of bound nucleons'
       write(22,*) '# 5: number of all particles / nEns'
       write(22,*) '# 6: net number of baryons / nEns'
       write(22,*) '# 7: number of nucleons / nEns'
       write(22,*) '# 8: number of bound nucleons'
...
!             dE = teilchen(i,j)%perWeight*(/1.,EE,0./)+(/0.,0.,1./)
       ...
       if (EE_nucB(3).eq.0.) cycle
       call AddHist(hBA,EE_nucB(2)/EE_nucB(1)-mN,EE_nucB(1)/EE_nucB(3))

    End do Loop_over_ensembles

    ! Printout the Energy per baryon:
    write(22,fmt='(f8.2,4(1x,es13.6),4(1x,f10.3))') time,&
         & E_all(2)/E_all(1), E_bar(2)/E_bar(1), E_nuc(2)/E_nuc(1), &
         & E_nucB(2)/E_nucB(1), &
         & E_all(3)/nEns,E_bar(1)/nEns,E_nuc(1)/nEns,E_nucB(1)/nEns
    close(22)
  ...
  end subroutine evaluateBindingEnergy_theiss
...
!   end subroutine checkTrajectories
  ...
  subroutine evaluateTimeStep(iflag,coll_num,delta_T_max,time,delta_T_new)
    ...
    integer, dimension(0:1,0:3,1:2) :: CountedEvents
    ...
    CountedEvents=GetCountedEvents()
    ...
    if(iflag.eq.1) then
       num_2body=2*CountedEvents(1,2,1)
       num_part=n_participants(1)
    else if(iflag.eq.2) then
       num_2body=CountedEvents(1,2,2)
