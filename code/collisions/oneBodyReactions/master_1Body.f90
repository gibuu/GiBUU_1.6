...
module master_1Body
  ...
  Public :: decayParticle, assignCharge
...
contains
  ...
  subroutine decayParticle (resonanceIN, finalState, collisionFlag, pauliFlag, finalFlag, time, gammaOut)
    ...
    integer :: i, anzahl, dId, antiCharge, antiID, stability
    ...
    collisionFlag = setKinematics (resonance, finalState(1:anzahl), mediumAtPosition, betaToLRF)
    ...
    finalState%history      = setHistory (resonance)
...
!!$    end do
  ...
  ! NOTES
  ! It's important that the Id's and charges of the "finalState" 
  ...
  function setKinematics (resonance, finalState, mediumAtCollision, betaToLRF) result(collisionFlag)
    ...
    do i=lBound(finalState,dim=1),uBound(finalState,dim=1)
       If (finalState(i)%Id <= 0) then
          write(*,*) 'SetKinematics[1]: Particle ID == 0!!!',finalState(lBound(finalState,dim=1):uBound(finalState,dim=1))%ID
          stop
       end if
    end do
    ...
    Case(2)
       energyCorrectLoop : Do i=1, maxCorrectLoop
          ...
          call massass(srts_vacuum,mediumAtCollision,pair,finalState,betaToLRF,betaToCM,flag)
          If (.not. flag) then
