...
!!$      MDCY(116,1)=1             ! decay: K+
      ...
      do i=1,N
         ...
         outPart(i)%scaleCS=1.
         outPart(i)%in_formation=.false.
         outPart(i)%event = Part%event
         outPart(i)%firstevent = Part%firstevent
         outPart(i)%perturbative = Part%perturbative
         outPart(i)%lastCollisionTime = time
         outPart(i)%perWeight = Part%perWeight
         outPart(i)%history = setHistory (Part)
      end do
