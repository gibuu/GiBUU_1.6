...
contains
  ...
  function winkel (teilchenIn, teilchenOut, srts, betaToCM, mediumAtCollision, successflag) result (pscatt)
    ...
    if (id2 == 0) then 
       ! ******************PARTICLE DECAYS*************************
       If (id1==Delta .and. deltaPWave) then
...
!    write(*,'(0P,3f11.3)') pscatt
  ...
  function pscatt_BarBar (teilchenIn, teilchenOut, srts, betaToCM, successflag) result(pscatt)
    ...
    real :: bb,a,ta,x,t,c1,c1_min,c1_max,asrt,as,t1,t2,s1,c2,s2,ct1,st1,ct2,st2,ss
    ...
    if (id1==Nucleon .and. id2==Nucleon .and. id3==Nucleon .and. id4==Nucleon) then
       ...
       if (teilchenIn(1)%antiParticle.eqv.teilchenIn(2)%antiParticle) then
          ...
          case (1)
             ...
             if (x<=1./(1.+a)) then
                x = rn()
                t = alog( x + exp(bb*ta)*(1.-x) ) / bb
             else
                x = rn()
                t = ta - alog( 1. - x*(1.-exp(bb*ta)) )/bb
             endif
             c1 = 1. - 2.*t/ta
          case (0)
  ...
  !
  ! Here b is normalization constant, and the parameter a is taken from a fit
  ...
  function dsigdt_NRes (srts, mRes) result (cost)
    ...
    if (abs(a-1.)<1E-3) then
      t = tmin * exp(rn() * log(tmax/tmin))
    else
      t = - ( rn()*(abs(tmax)**(1.-a)-abs(tmin)**(1.-a)) + abs(tmin)**(1.-a) ) ** (1./(1.-a))
    end if
