...
contains
  ...
  ! NAME
  ! subroutine massAss(srts,mediumAtColl,PartIn,PartOut,betaToLRF,betaToCM,flagOK)
  !
  ! PURPOSE
  ...
  subroutine massAss(srts,mediumAtColl,PartIn,PartOut,betaToLRF,betaToCM,flagOK)
    ...
    if (PartIn(1)%ID==nucleon .and. PartIn(2)%ID==nucleon .and.   &
      ...
      call massAss_Full(srts,mediumAtColl,PartIn,PartOut,betaToLRF,betaToCM,flagOK)
    end if
  ...
  subroutine massAss_NN_NDelta (srts, mediumAtColl, PartIn, PartOut, betaToCM, flagOK)
    ...
    if(.not.partIn(1)%antiParticle .and. .not.partIn(2)%antiParticle) then
       call massNN_NDelta(srts,massDelta)
    else if(partIn(1)%antiParticle .neqv. partIn(2)%antiParticle) then
       call massNbarN_NbarDelta(srts,massDelta)
    else
