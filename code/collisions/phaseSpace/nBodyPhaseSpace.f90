...
! for the simulation of the particle momenta according to the n-body phase space.
!*************************************************************************
module nBodyPhaseSpace
...
contains
  ...
  !   (first index : particle, second index : momentum index)
  ! NOTES
  ...
  subroutine momenta_in_nBodyPS (srts, mass, pn)
    ...
    if (n < 2 .or. n > 6) then
       ...
       stop
    else if (size(pn,dim=1)/=n .or. size(pn,dim=2)/=3) then
    ...
    case(2)
       ...
       pn(1:2,1:3) = momenta_in_2BodyPS (srts, mass(1:2))
    ...
    case(4)
       ...
       pn(1:4,1:3) = momenta_in_4BodyPS (srts, mass(1:4))
       if (debugFlag) write(*,*)' Done'

    case(5)
       if (debugFlag) write(*,*)' Calling momenta_in_5BodyPS ...'
       pn(1:5,1:3) = momenta_in_5BodyPS (srts, mass(1:5))
       if (debugFlag) write(*,*)' Done'

    case(6)
       if (debugFlag) write(*,*)' Calling momenta_in_6BodyPS ...'
       pn(1:6,1:3) = momenta_in_6BodyPS (srts, mass(1:6))
       if (debugFlag) write(*,*)' Done'
  ...
  ! OUTPUT
  ! * real, dimension(2,3) :: p2 -- 2 three-vectors of momenta
  !   (first index : particle, second index : momentum index)
  ! NOTES
  !*************************************************************************
  function momenta_in_2BodyPS (srts, mass) result (p2)
  ...
  function momenta_in_3BodyPS (srts, mass) result (p3)
    ...
    p3(1,1)=sint3*cos(phi)*pp3(1)
    p3(1,2)=sint3*sin(phi)*pp3(1)
    p3(1,3)=cost3*pp3(1)

    ! The second momentum vector is now defined by momentum conservation:
    p3(2,1:3)=-p3(1,1:3)-p3(3,1:3)

    !...check:
    if (abs(p3(2,1)**2+p3(2,2)**2+p3(2,3)**2-pp3(2)**2)>1e-05) then
       write(*,*)'Problems in momenta_in_3BodyPS:', &
                  p3(2,1),p3(2,2),p3(2,3),pp3(2),p3(2,1)**2+p3(2,2)**2+p3(2,3)**2-pp3(2)**2
       write(*,*)'srt(s)=',srts
       stop
    end if
  ...
  ! OUTPUT
  ! * real, dimension(4,3) :: p4 -- 4 three-vectors of momenta
  !   (first index : particle, second index : momentum index)
  ! NOTES
  ...
  function momenta_in_4BodyPS (srts, mass) result (p4)
    ...
    ptot=0
    do j=1,4
       ptot=ptot+sqrt(mass(j)**2+p4(j,1)**2+p4(j,2)**2+p4(j,3)**2)
    end do
  ...
  ! OUTPUT
  ! * real, dimension(5,3) :: p5 -- 5 three-vectors of momenta
  !   (first index : particle, second index : momentum index)
  ! NOTES
  ...
  function momenta_in_5BodyPS (srts, mass) result (p5)
    ...
    p5(5,1:3)=pfinal*rnOmega()

    !... determine momenta of particles 1-4 in their rest frame:
    p5(1:4,1:3) = momenta_in_4BodyPS (q, mass(1:4))

    !... boost momenta to overall rest frame:
    e1234=sqrt(q2+pfinal**2)
    beta(1:3) = p5(5,1:3)/e1234

    do k=1,4
       fourVector(1:3)= p5(k,1:3)
       fourVector(0)  = sqrt(mass(k)**2+Dot_Product(fourVector(1:3),fourVector(1:3)))
       call lorentz(beta,fourVector)
       p5(k,1:3) = fourVector(1:3)
    end do
    ...
    ptot=0.
    do k=1,5
       ptot=ptot+sqrt(mass(k)**2+p5(k,1)**2+p5(k,2)**2+p5(k,3)**2)
    end do
  ...
  ! OUTPUT
  ! * real, dimension(6,3) :: p6 -- 6 three-vectors of momenta
  !   (first index : particle, second index : momentum index)
  ! NOTES
  ! Uses the recursion formula for the phase space (see PDG-2002, p.269)
  ! in a similar way as in subroutine momenta_in_4BodyPS.
  ...
  function momenta_in_6BodyPS (srts, mass) result (p6)
    ...
    p6(6,1:3)=pfinal*rnOmega()

    !... determine momenta of particles 1-5 in their rest frame:
    p6(1:5,1:3) = momenta_in_5BodyPS (q, mass(1:5))

    !... boost momenta to overall rest frame:
    e12345=sqrt(q2+pfinal**2)
    beta(1:3) = p6(6,1:3)/e12345

    do k=1,5
       fourVector(1:3)= p6(k,1:3)
       fourVector(0)  = sqrt(mass(k)**2+Dot_Product(fourVector(1:3),fourVector(1:3)))
       call lorentz(beta,fourVector)
       p6(k,1:3) = fourVector(1:3)
    end do
    ...
    ptot=0.
    do k=1,6
       ptot=ptot+sqrt(mass(k)**2+p6(k,1)**2+p6(k,2)**2+p6(k,3)**2)
    end do
    if (abs(ptot-srts)>1.e-05) then
  ...
  function integrate_nBodyPS (srts, mass) result (phi)
    ...
    real, intent(in) :: srts                ! c.m. energy (GeV),
    real, intent(in), dimension(:) ::  mass ! masses of outgoing particles (GeV)
    real :: phi                             ! phase space volume (GeV**(2*n-4))
