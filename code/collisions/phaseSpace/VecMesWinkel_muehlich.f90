...
contains
  ...
  subroutine SelectCost(srts,idv,idb,fn,massv,massb,cost)
    ! selects scattering angle for gamma N -> V N / V Delta (low energy)
    ! calls: photorho,photomega,photophi,photodel
    use random, only: rn
    use twoBodyTools, only: pcm
    implicit none
    real,intent(in)::srts    ! sqrt(s)
    real,intent(in)::massv   ! mass of outgoing vector meson
    real,intent(in)::massb   ! mass of outgoing baryon
    integer,intent(in)::idv  ! ID of outgoing vector meson
    integer,intent(in)::idb  ! ID of outgoing baryon
    integer,intent(in)::fn   ! charge of outgoing baryon
    real,intent(out)::cost   ! result: cos(t)
...
!           call photorho(s,tmin,massv,fn,m2max)
        if(ivec==2)then
          call photomega(s,tmin,massv,fn,m2max)
        else if(ivec==3)then
          call photophi(s,tmin,massv,fn,m2max)
        end if
!     else if(idb==2)then
...
!               call photorho(s,t,massv,fn,m2)
          if(ivec==2)then
              call photomega(s,t,massv,fn,m2)
          else if(ivec==3)then
              call photophi(s,t,massv,fn,m2)
          end if
!         else if(idb==2)then
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine photomega(s,t,mv,inuc,msq)
  ...
  contains
    ...
    function resform(x,cutoff)
      ...
      real::mass
      mass=mres
      resform=cutoff**4/(cutoff**4+(x-mass**2)**2)
    end function resform
     
    function form_tchannel(t)
  ...
  end subroutine photomega

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  subroutine photophi(scms,t,mp,fn,msq)
    ...
    real,intent(out)::msq
    real::pin,pout,tmin,dsig,u,s1,s2!,kcm
    real alph10,alph20,alphb,alph1,msqr!,sth
    parameter(alph10=1.08,alph20=-0.75,alphb=0.25)     
    complex x1,x1d,x3,x4,x5,x3d,x5d,sum!,x4d
    real gpinn,gphigapi,meta,getann,gphigaeta
    parameter(gphigapi=-0.141,meta=0.547)
    parameter(getann=-3.527,gphigaeta=-0.707)
    real c1,c2,f,f1,bslope
    parameter(c1=2.09,s1=1.6,bslope=1.,c2=0,s2=1)
    real kp,kq,pq
    complex y1,y2,y3,y4,gphinn,kapan,kapaphi,sum6
...
!    x4d=conjg(x4)
    ...
    sum6=(2*mn**4*(2*mp**2 - t)*x1*x1d - t*(2*scms**2 + 2*scms*t +     &
         ...
         t**3*x3*x3d))/mp**2
    sum=sum6+sum2(scms,t,u,y1,y2,y3,y4,fn)+sum1(scms,t,u,y1,y2,y3,y4,x1,x5,fn)
    msqr=real(sum)
  ...
  complex function sum1(s,t,u,y1,y2,y3,y4,x1,x3,qn)
      implicit none
      integer qn
      real s,t,u,fn
      ...
      complex part1,part2,part3,part4
...
!      y4d=conjg(y4)
  ...
  complex function sum2(s,t,u,y1,y2,y3,y4,qn)
      integer qn
      real s,t,u,fn
      real, parameter :: mp = 1.02
