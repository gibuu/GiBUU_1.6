...
! choose the momentum and masses in a final state prescription. Read
! chapter 4.7 of Lehr's Dr. thesis to understand the principles of this 
! decision.
!***************************************************************************
module finalState_Full
...
CONTAINS
  ...
  subroutine massAss_Full (srts, medium_AtCollision, pairIn, pairOut, betaToLRF, betaToCM, successFlag)
    ...
    real, dimension(1:2,1:3)  :: momentum        ! CM momentum of outgoing particles
  ...
  contains
    ...
    !***********************************************************************
    subroutine getMaximum(maxBWD)
      use IdTable, only: nucleon, Delta, nres, nsres, photon, pion, rho, omegaMeson, phi, D35_2350, D35_1930, D13_1520, S11_1535
      ...
      if (((pairIn(1)%ID.eq.photon).or.(pairIn(2)%ID.eq.photon)) &
      ...
      else if (pairIn(1)%Id.le.nres+1 .and. pairIn(2)%Id.eq.0) then
         ...
         channelNumber=12
         if(.not.channels(channelNumber)%init) &
              & channels(channelNumber)=initChannel('VM + stable in vacuum',maxbwd*maxbwd_scalingFactor)
      ...
      else if (pairIN(1)%ID==nucleon .and. pairIN(2)%ID==nucleon .and. pairOUT(1)%ID==Delta .and. pairOUT(2)%ID==Delta) then
         ! NN -> Delta Delta
         maxbwd = 28.
         channelNumber = 14
         if (.not.channels(channelNumber)%init) &
           channels(channelNumber)=initChannel('NN -> DD',maxbwd*maxbwd_scalingFactor)
      else if (pairIN(1)%ID==nucleon .and. pairIN(2)%ID==nucleon .and. &
               pairOUT(1)%ID==nucleon .and. pairOUT(2)%ID==D13_1520) then
         ! NN -> N D13_1520
         maxbwd = 6.
         channelNumber = 15
         if (.not.channels(channelNumber)%init) &
           channels(channelNumber) = initChannel('NN -> N D_13(1520)',maxbwd*maxbwd_scalingFactor)
      else if (pairIN(1)%ID==nucleon .and. pairIN(2)%ID==nucleon .and. &
               pairOUT(1)%ID==Delta .and. pairOUT(2)%ID==D13_1520) then
         ! NN -> Delta D13_1520
         maxbwd = 26.
         channelNumber = 16
         if (.not.channels(channelNumber)%init) &
           channels(channelNumber) = initChannel('NN -> D D_13(1520)',maxbwd*maxbwd_scalingFactor)
      else if (pairIN(1)%ID==nucleon .and. pairIN(2)%ID==nucleon .and. &
               pairOUT(1)%ID==Delta .and. pairOUT(2)%ID==S11_1535) then
         ! NN -> Delta S11_1535
         maxbwd = 19.
         channelNumber = 17
         if (.not.channels(channelNumber)%init) &
           channels(channelNumber) = initChannel('NN -> D S_11(1535)',maxbwd*maxbwd_scalingFactor)
      else
         ! default
         maxbwd = 4.
         channelNumber = 18
    ...
    ! 
    ! OUTPUT
    ! * real, dimension(1:2,1:3) :: momentum -- of the final state
    !   (first index : particle, second : momentum component)
    ...
    subroutine throwDice (success, momentum, mass, massFailure_Nuc, treatNuc_onshell)
      ...
      real, dimension(1:2,1:3),intent(out) :: momentum
      logical, intent(out) :: success, massFailure_nuc
      logical, intent(in)  :: treatNuc_onshell

      real :: maxValue,probability,p_cd_vacuumMax,p_cd,helper
      ...
      if (.not.(flagstable(1).and.flagstable(2))) then
         ...
         momLoop : do momSteps=1,maxMomSteps
            ...
            If (p_cd/p_cd_vacuumMax>=1.) then
               write(*,*) "Problem with p_cd's in massAss", p_cd,p_cd_vacuumMax
            end if
            ...
            probability=p_cd/p_cd_vacuumMax * helper /maxValue
  ...
  contains
    ...
    ! subroutine checkMaximum(value, maxValue)
    ! PURPOSE
    ! This subroutine checks wether the maximal value is really the maximal value. 
    ! If not, then error messages are written to standard out and
    ! 'massAssError.dat'. After a critical number of errors the program is terminated!!
    !******************************************************************************
    subroutine checkMaximum(value, maxValue)
      use baryonWidthMedium, only: get_MediumSwitch_coll
    ...
    ! PURPOSE
    ! We consider a + b -> c + d + e. The kinematics of c , d & e are determined.
    ! Here the masses and momenta of the particles are choosen by Monte-Carlo decision. 
    ...
    ! We then decide to reject or accept the final state. If refjected new masses and momenta are choosen until
    ! we find a final state that is accepted.
    ! If it's not possible to find a final state, than successs=.false. is set and the routine is terminated.
    !******************************************************************************
    subroutine throwDice (maxValue, success, momentum, mass)
      ...
      real , dimension(1:3,1:3),intent(out)  :: momentum  ! first index : particle, second : momentum component
      ...
      if (flagstable(1).and.flagstable(2).and.flagstable(3)) then
         ...
         outerloop : do outerSteps=1,maxOuterSteps
            ...
            momentum(1,1)=sinTheta_13*cos(phi)*pCmAbs(1)
            momentum(1,2)=sinTheta_13*sin(phi)*pCmAbs(1)
            momentum(1,3)=CosTheta_13*pCmAbs(1)
            ...
            cosTheta=2.*(rn()-0.5)
            Do k=1,3
               momentum(k,1:3) = rotateYZ (0.0, phi, momentum(k,1:3), cosTheta=cosTheta)
            End do
  ...
  ! * type(particle), dimension(:) :: finalState      ! Masses and four-momenta and of the final state 
  !                                                   ! particles
  ! NOTES
  ...
  real, intent(in) :: srts                                    ! Invariant energy
  real, dimension(1:3), intent(in) :: betaToLRF               ! Boost from computational frame to LRF
  real, dimension(1:3), intent(in) :: betaToCM                ! Boost from computational frame to CM frame
  type(medium), intent(in) :: mediumAtCollision               ! Medium info at collision point
  ...
  if(flagAllStable) then  ! Only stable outgoing particles 
     ...
     call momenta_in_nBodyPS(srts,finalState(1:n)%mass,pn(1:n,1:3))
     do k=1,n
        finalState(k)%momentum(1:3)=pn(k,1:3)
        finalState(k)%momentum(0)=sqrt(finalState(k)%mass**2+dot_product(pn(k,1:3),pn(k,1:3)))
     end do
     success=.true.
     return
  end if          
...
!  Testing:
  ...
  attemptLoop : do i=1,maxAttempts
      ...
      if(flagMomDep) then
         ! Monte-Carlo sampling of the phase space:
         call momenta_in_nBodyPS(srts,mass(1:n),pn(1:n,1:3))
      else
         pn(1:n,1:3)=0.
      end if
  ...
  if(.not.flagMomDep) call momenta_in_nBodyPS(srts,mass(1:n),pn(1:n,1:3))
  ...
  do k=1,n
     ...
     finalState(k)%momentum(1:3)=pn(k,1:3)
     finalState(k)%momentum(0)=sqrt(mass(k)**2+dot_product(pn(k,1:3),pn(k,1:3)))
  end do
