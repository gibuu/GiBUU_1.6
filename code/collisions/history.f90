...
!
! In particular, we safe the particle ID's of those particles which produced 
! the particle. E.g. if a pion was produced in AB -> X pion, then we store 
! the ID's of the two incoming  particles "A" and "B". 
!
...
module history
  ...
  ! PURPOSE
  ! This flag determines whether we will increase the stored
  ! 'generation' of particles in an elastic collision.
  !*************************************************************************
...
1003 FORMAT ('--- *** namelist "',A,'" missing in jobcard. Values-->Default!')
  ...
  integer function setHistory1 (p1)
    ...
    setHistory1 = setHistory1 + generation*million

  end function setHistory1



  integer function setHistory2 (p, final)
    use particleDefinition
    type(particle), dimension(2), intent(in) :: p
    type(particle), dimension(:), intent(in), optional :: final
    integer :: id1, id2, generation
    logical :: elast

    if (initFlag) call readInput

    generation = max (abs(p(1)%history), abs(p(2)%history)) / million

    elast = .false.
    if (present(final)) then
      if (final(3)%ID<=0 .and. IsElastic(p,final(1:2))) elast = .true.
    end if

    if ((.not.elast) .or. IncGeneration_Elastic) generation = generation + 1
    ...
    setHistory2 = id1 + 1000*id2 + generation*million

  end function setHistory2



  integer function setHistory3 (p1, p2, p3)
    ...
    if ((p1%ID==nucleon).and.(p2%ID==nucleon).and.(p3%ID==nucleon)) then      
      ...
      setHistory3 = -nnDelta  ! ---- NNDelta channel
      ...
      setHistory3 = -nnPion   ! ---- NNpi channel
    else
      setHistory3 = -some3Body
    end if

    setHistory3 = setHistory3 - generation*million

  end function setHistory3
