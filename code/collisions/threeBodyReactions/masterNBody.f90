...
!            &sum(finalState(:)%momentum(2)),sum(finalState(:)%momentum(3))
  ...
  contains
    ...
    real, dimension(3,3)  :: p3      ! first index : particle, second index : momentum index
    ...
    case(1)
      ...
      particle1%momentum(1:3)=p3(1,1:3)      
      particle2%momentum(1:3)=p3(2,1:3)      
      particle3%momentum(1:3)=p3(3,1:3)
...
! Statistical variables (do not influence dynamics):
  ...
  integer, save, dimension(numQ,2:Nmax+2) :: &
          & num_Nbody_collisions_vs_Q_pair, num_Nbody_collisions_vs_Q_cluster
    ...
    if(flag) then
      ...
      if(time.ge.time_max) then
         ...
         do i=1,size(num_Nbody_collisions_vs_Q_pair,dim=1)
           ...
           write(36,'(f7.3,100(1x,i5))') Q_pair,&
                                      &num_Nbody_collisions_vs_Q_pair(i,:)
         end do
         ...
         do i=1,size(num_Nbody_collisions_vs_Q_cluster,dim=1)
           ...
           write(36,'(f7.3,100(1x,i5))') Q_cluster,&
                                      &num_Nbody_collisions_vs_Q_cluster(i,:)
         end do
