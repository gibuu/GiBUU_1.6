...
module collisionNumbering
  ...
  Public :: check_justCollided, getCountedEvents, nullCountedEvents
  Public :: getn_participants, nulln_participants
  Public :: real_firstnumbering

  integer, save :: n_participants_rr=0,n_participants_rp=0 
...
contains
  ...
  !
  ! OUTPUT
  ! * integer, dimension(0:1,0:3,1:2) :: getCountedEvents
  !*************************************************************************
  function getCountedEvents() result(a)
    integer, dimension(0:1,0:3,1:2) :: a
    a=countedEvents
...
!...
    ...
    else if(code1.eq.12) then
      ! perturbative particle decay:
      CountedEvents(:,1,2) = CountedEvents(:,1,2)+1
      CountedEvents(:,0,2) = CountedEvents(:,0,2)+1
    else if(code1.eq.211) then
      ! real+real collision:
      CountedEvents(:,2,1) = CountedEvents(:,2,1)+1
      CountedEvents(:,0,1) = CountedEvents(:,0,1)+1
    else if(code1.eq.212) then
      ! real+perturbative collision:
      CountedEvents(:,2,2) = CountedEvents(:,2,2)+1
      CountedEvents(:,0,2) = CountedEvents(:,0,2)+1
    else if(code1.eq.3111) then
      ! real+real+real collision:
      CountedEvents(:,3,1) = CountedEvents(:,3,1)+1
      CountedEvents(:,0,1) = CountedEvents(:,0,1)+1
    else if(code1.eq.3112) then
      ! real+real+perturbative collision:
