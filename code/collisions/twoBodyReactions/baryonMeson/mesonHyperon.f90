...
contains
  ...
  !***************************************************************************
  subroutine mesonY(srts,teilchenIN,mediumATcollision,momentumLRF,teilchenOUT,sigmaTot,sigmaElast,plotFlag)
  use idTable, only: pion, eta, kaonBar, nucleon, Delta, Lambda, lambda_1600, SigmaResonance, SigmaStar, sigma_1915, nbar
  ...
  ! partial cross sections for meson Y -> R
  ...
  ! (1) Check  Input
  if((teilchenIn(1)%Id.eq.pion.or.teilchenIn(1)%Id.eq.eta)&
    &.and.teilchenIn(2)%Id.ge.Lambda&
    &.and.teilchenIn(2)%Id.le.sigma_1915) then
    meson_particle=teilchenIn(1) ;   hyperon_particle=teilchenIn(2)
  else if((teilchenIn(2)%Id.eq.pion.or.teilchenIn(2)%Id.eq.eta)&
    &.and.teilchenIn(1)%Id.ge.Lambda&
    &.and.teilchenIn(1)%Id.le.sigma_1915) then
    meson_particle=teilchenIn(2) ;   hyperon_particle=teilchenIn(1)
  else 
  ...
  If(Sum(teilchenOut(:)%Charge)&
    &.ne.hyperon_particle%charge+meson_particle%charge) then
  ...
  contains
    ...
    if(meson_particle%Id.eq.pion) then
      ...
      if (srts > mN + mK) then
        ...
        if(hyperon_particle%Id.eq.Lambda) then
          !* Lambda pi -> N Kbar:     
          sigbg=slaka(isrts)
        else if(hyperon_particle%Id.eq.SigmaResonance) then
          !* Sigma pi -> N Kbar:
          sigbg=ssigka(3*qhyperon+qmeson,1,isrts)
        else if(hyperon_particle%Id.ge.lambda_1600.and.&
               &hyperon_particle%Id.le.sigma_1915) then
          !* Y* pi -> N Kbar:
          pinitial = pCM(srts,hyperon_particle%mass,meson_particle%mass)  ! Initial c.m. momentum
          pfinal   = pCM(srts,mN,mK)                                      ! Final c.m. momentum
          if(abs(qhyperon+qmeson).eq.2) then
            isoFactor=0.
          else if(hadron(hyperon_particle%Id)%isoSpinTimes2.eq.0) then
            isoFactor=1.
          else if(qhyperon+qmeson.eq.0) then
            if(qmeson.eq.0) then
              isoFactor=1./3.
            else
              isoFactor=5./6.
      ...
      ! No background for eta Lambda collisions:
      sigbg=0.
  ...
  !  real, dimension(-4:4,-1:1,0:nssmmax), save ::  ssigka,ssigka_res
    ...
    subroutine makeDecision
      ...
      if (sigmaRes_tot >= cut) then
        ...
        else if(totalCharge.eq.-1) then
          teilchenOut(1)%Charge=-1
          teilchenOut(2)%Charge=0
        else 
          if(hyperon_particle%Id.eq.SigmaResonance) then
            ! pi Sigma -> Kbar N:          
            sum=0.
            ...
            teilchenOut(1)%Charge=qkaon
            teilchenOut(2)%Charge=-qkaon
          else
            x=rn()
            if(x.le.0.5) then
              teilchenOut(1)%Charge=-1
    ...
    subroutine makeOutPut
      ...
      ! The output files
      character(30), dimension(1:4) :: outputFile
      real :: plab,plab_Lambda,plab_Sigma
      real :: sigmaElast_res,sigmaKbarN_res
      integer :: isrts
 
      outputFile(1)='mesonY_sigTotElast.dat'
      outputFile(2)='mesonY_KbarN.dat'
      outputFile(3)='pionLambda_KbarN.dat'
      outputFile(4)='pionSigma_KbarN.dat'
