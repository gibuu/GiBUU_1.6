...
module master_2Body
  ...
  !********************************************************************
  !****g* master_2Body/smoothHiEnergy
  ! SOURCE
  !
  logical, save :: smoothHiEnergy = .true.
  ! PURPOSE
  ! Flag to indicate, whether a smooth transition from the resonance
  ! prescription to the HiEnergy routines should be performed or not.
  !********************************************************************
  ...
  ! PURPOSE
  ! Flag, whether to use Fritiof or Pythia for high energetic collisions
  ! (except for baryon-antibaryon channel, where separate switch is used now):
  ! * 0: Use Fritiof
  ...
  !********************************************************************
  !****g* master_2Body/usePythiaCut
  ! SOURCE
  !
  real,save :: usePythiaCut = 2.
  ! PURPOSE
  ! cut-off sqrt(s): above this value PYTHIA is usable directly
  ! (for all channels except for baryon-antibaryon, where separate cut-off is used now)
  !********************************************************************
  ...
  ! PURPOSE
  ! Flag, whether to use Fritiof or Pythia for high energetic
  ...
  !********************************************************************
  !****g* master_2Body/usePythiaCut_BaB
  ! SOURCE
  !
  real,save :: usePythiaCut_BaB = 2.
  ! PURPOSE
  ! cut-off sqrt(s): above this value PYTHIA is usable directly
  ! for the  baryon-antibaryon collisions
  !********************************************************************

  !********************************************************************
  !****g* master_2Body/doPythiaReweight
  ! SOURCE
  !
  ...
  ! PURPOSE
  ! Choice of angular distribution in elastic collisions
  ! (cf. DoColl_Elast):
  ! * 1 = isotropic
  ...
  ! PURPOSE
  ! if .true. ---  set constant energy independent elastic baryon-baryon
...
contains
  ...
  subroutine readInput
    ...
    ! * coulombCorrect
    !********************************************************************
    NAMELIST /master_2Body/ correctEnergy, baryonBaryonScattering, baryonMesonScattering, mesonMesonScattering, &
                            debug, usePythia, usePythiaCut, usePythia_BaB, usePythiaCut_BaB, doPythiaReweight, &
                            useHiEnergy, HiEnergyThresholdBarMes, HiEnergyThresholdBarMesDelta, HiEnergyThresholdBarBar, &
                            HiEnergyThresholdBarBarDelta, HiEnergyThresholdBarAntibar, HiEnergyThresholdBarAntibarDelta, &
                            smoothHiEnergy, useManni, ElastAngDist, flagElastBB, coarse, &
                            bmax_nucleonNucleon, bmax_nucleonResonance, bmax_hyperonNucleon, bmax_baryonPion, &
    ...
    character(60), dimension(0:2) :: NN
    character(60), dimension(1:3) :: NNe
    integer :: ios

    NN(0)='No'
    NN(1)='Use Pythia for ALL collisons above UsePythiaCut'
    NN(2)='Use Pythia for PRIMARY collisons above UsePythiaCut'

    NNe(1)='isotropic'
    NNe(2)='J. Cugnon et al., NPA 352, 505 (1981)'
    NNe(3)='Pythia (default)'
    ...
    write(*,*) '  smooth transition to HiEnergy    : ',smoothHiEnergy
    write(*,'(A,f5.2," +-",f5.2," GeV")') ' srts threshold bar-mes    :', HiEnergyThresholdBarMes, HiEnergyThresholdBarMesDelta
    write(*,'(A,f5.2," +-",f5.2," GeV")') ' srts threshold bar-bar    :', HiEnergyThresholdBarBar, HiEnergyThresholdBarBarDelta
    write(*,'(A,f5.2," +-",f5.2," GeV")') ' srts threshold bar-antibar:', HiEnergyThresholdBarAntibar, &
                                                                          HiEnergyThresholdBarAntibarDelta

    write(*,*) '  use Manni                        : ',useManni

    if ((usePythia<0).or.(usePythia>2)) then
       write(*,*) '  use PYTHIA directly              : ',usePythia,' !!!! STOP'
       stop
    endif

    write(*,'(A,i3," = ",A)') '   use PYTHIA directly              : ',usePythia, NN(usePythia)
    if (usePythia>0) then
       write(*,*) '  use PYTHIA directly above sqrt(s)= ',usePythiaCut
       write(*,*) '  use PYTHIA with reweighted events: ',doPythiaReweight
       if (doPythiaReweight) then
          write(*,*) '!!!!! When using PYTHIA within the reweighting mode, !!!!!'
          write(*,*) '!!!!! the event weights have to be adjusted!!!       !!!!!'
          write(*,*) '!!!!! This is done here very crude and only for      !!!!!'
          write(*,*) '!!!!! N+N, pi+N, K+N @ sqrt(s) = 10..40 GeV.         !!!!!'
          if (usePythiaCut < 10.0) then
             write(*,*) '!!!!! Resetting:'
             usePythiaCut = 10.0
             write(*,*) '  use PYTHIA directly above sqrt(s)= ',usePythiaCut
          endif
    ...
    if ((usePythia_BaB<0).or.(usePythia_BaB>2)) then
       write(*,*) '  use PYTHIA directly for BaB channel : ',usePythia_BaB,' !!!! STOP'
       stop
    endif

    write(*,'(A,i3," = ",A)') '   BaB: use PYTHIA directly         : ',usePythia_BaB, NN(usePythia_BaB)
    write(*,*) '                  ... above sqrt(s)= ',usePythiaCut_BaB

    write(*,*)

    write(*,'(A,3(1x,f5.2))') '  coarse maximum distances for bar-bar, bar-mes , mes-mes, fm : ',coarse
    write(*,'(A,f5.2)')       '  max. impact parameter for nucleon-nucleon scatt.,  fm :', bmax_nucleonNucleon
    write(*,'(A,f5.2)')       '  max. impact parameter for nucleon-resonace scatt., fm :', bmax_nucleonResonance
    write(*,'(A,f5.2)')       '  max. impact parameter for hyperon-nucleon scatt.,  fm :', bmax_hyperonNucleon
    write(*,'(A,f5.2)')       '  max. impact parameter for baryon-pion scatt.,      fm :', bmax_baryonPion
    write(*,'(A,f5.2)')       '  max. impact parameter for baryon-meson scatt.,     fm :', bmax_baryonMeson
    write(*,'(A,f5.2)')       '  max. impact parameter for meson-meson scatt.,      fm :', bmax_mesonMeson
    write(*,*)' flagElastBB: ', flagElastBB

    if(OverideSigma_PiN.ge.0.0) then
       write(*,'(/,A,g12.3,A,/)') '!!!!! YOU HAVE SET PION NUCLEON CROSS SECTION BY HAND: sigma = ',OverideSigma_PiN,' mb !!!!!'
    end if
    if(OverideSigma_RhoN.ge.0.0) then
       write(*,'(/,A,g12.3,A,/)') '!!!!! YOU HAVE SET RHO NUCLEON CROSS SECTION BY HAND:  sigma = ',OverideSigma_RhoN,' mb !!!!!'
    end if

    if(OverideSigma_PiPi.ge.0.0) then
       write(*,'(/,A,g12.3,A,/)') '!!!!! YOU HAVE SET PION PION CROSS SECTION BY HAND: sigma = ',OverideSigma_PiPi,' mb !!!!!'
    end if
    ...
    if (smoothHiEnergy) then
      ...
      call setMaxSqrts_2bps (HiEnergyThresholdBarBar)
      call setMaxSqrts_dimi (HiEnergyThresholdBarBar)
    end if
  ...
  ! EventWeight, collision_time, weightLocal, sigTot_out, pauliIncluded_out)
  !
  ! PURPOSE
  ...
  subroutine collide_2body (pair, finalState, time, collisionFlag, HiEnergyFlag, HiEnergyType, &
    ...
    real, optional,intent(out)                :: EventWeight, collision_time
    ...
    If (sqrtS_vacuum < Sum(pair%mass)) then
       write(*,*) '***************************'
       write(*,*) 'Error in master_2body: sqrtS_vacuum.lt.Sum(pair%mass): sqrtS_vacuum',sqrtS_vacuum,Sum(pair%mass)
    ...
    call generateFinalState (pair, finalState, stringFactor, numEnsembles, time, collisionFlag, &
                             HiEnergyFlag, HiEnergyType, weightLocal, scenario, XShelp, PauliIncluded_out=PauliIncluded)

    if (PRESENT(EventWeight)) then
       EventWeight = PythiaWeight
    end if
    ...
    finalState%history = setHistory (pair, finalState)
  ...
  !   Used for reweighting the probability in the collision criteria. Not necessary in case of localEnsemble=.false.
  ...
  subroutine generateFinalState (pair, finalState, stringFactor, numEnsembles, time, &
                                 collisionFlag, HiEnergyFlag, HiEnergyType, weightLocal, scenario0, sigTot_out, PauliIncluded_out)
    ...
    integer, intent(out) :: HiEnergyType
    integer, optional,intent(in) :: weightLocal  ! Weight for the rescaling of the collision probability in case of "local" collisionCriteria
    ...
    If(srtS_vacuum.lt.Sum(pair%mass)) then
       write(*,*) 'Error in master_2body: srtS_vacuum.lt.Sum(pair%mass): srtS_vacuum',Sum(pair%mass),srtS_vacuum
       write(*,*) pair%mass, pair%ID, pair%charge
       write(*,*) pair%perturbative, pair%antiParticle
       write(*,*) pair(1)%momentum, pair(1)%velocity
       write(*,*) pair(2)%momentum, pair(2)%velocity
    ...
    call XsectionMaster (srtS_XS, pair, mediumATcollision, momentum_LRF, chosenEvent, &
                         ...
                         HiEnergyFlag, PauliIncluded_out=pauliIncluded)
    ...
    ! (7.5) Calculate dilepton production via Bremsstrahlung for NN and piN collisions (only if dilepton analysis is enabled).
    ...
    If(.not.HiEnergyFlag) then  !**** NOT HIGH ENERGY ****
       ...
       if( isBaryon(pair(1)%Id) .and. isBaryon(pair(2)%Id) .and. &
          ...
          call setKinematics (srtS, srtS_XS, betaToLRF, betaToCM, mediumAtCollision, pair, finalState(1:maxID), collisionFlag)
       ...
       do  ! Loop over attempts to simulate a HiEnergy event with energy conservation
          ...
          call setToDefault(finalState)
          If(pair(1)%perturbative.or.pair(2)%perturbative) then
  ...
  subroutine setKinematics (srts, srts_vacuum, betaToLRF, betaToCM, medium_AtCollision, pairIn, finalState, collisionFlag, verbose)
    ...
    logical :: potFailure ! set by energycorrection if there is a threshold violation due to potentials which are neglected
  ...
  contains
    ...
    subroutine Do2Body
      ...
      energyCorrectLoop : Do i=1, maxCorrectLoop
         ...
         call massass(srts_vacuum,medium_AtCollision,pairIn,finalState(1:2),betaToLRF,betaToCM,flag)
...
!!$         call WriteParticle(6,-99,2,finalState(2))
    ...
    subroutine Do4Body
      ...
      Do i=1, maxCorrectLoop
         ...
         do j=1,4
            finalState(j)%momentum(1:3) = p4(j,1:3)
...
!!$      stop
  ...
  subroutine setKinematicsHiEnergy (srts, srts_vacuum, sigmaTot, sigmaElast, &
    ...
    use IdTable, only : nucleon, JPsi, isMeson, isBaryon
    use output, only : WriteParticle
    use RMF, only : getRMF_flag
    ...
    case(scenarioBarMes)
       ...
       lPythia = .false.
       select case(usePythia)
       case(1)
          if (srtscor.gt.usePythiaCut) lPythia=.true.
       case (2)
          if (srtscor.gt.usePythiaCut) then
             if (pairIn(1)%firstEvent.eq.0 .and. pairIn(2)%firstEvent.eq.0) lPythia=.true.
          end if
       end select
       ...
       if (lPythia) then
          ...
          call DoColl_Pythia(pairIn,finalState,collisionFlag,  srtscor,pcm,betacor, doPythiaReweight,PythiaWeight)
    ...
    case(scenarioBarBar)
       ...
       case (scenarioBarAntiBar)
          ...
          case (2) ! === Prod ===
             ...
             lPythia = .false.
             select case(usePythia_BaB)
             case(1)
                if (srtscor.gt.usePythiaCut_BaB) lPythia=.true.
             case(2)
                if (srtscor.gt.usePythiaCut_BaB) then
                   if (pairIn(1)%firstEvent.eq.0 .and. pairIn(2)%firstEvent.eq.0) lPythia=.true.
                end if
             end select
  ...
  subroutine XsectionMaster (srts, teilchenIn, mediumATcollision, momentumLRF, teilchenOut, &
    ...
    case (scenarioBarBar)
       if (present(ForceHiEnergy)) then
         ...
         HiEnergyFlag = DecideHiEnergy (srts, HiEnergyThresholdBarAntibar, HiEnergyThresholdBarAntibarDelta)
       else
    ...
    case (scenarioBarMes)
       ...
       call BarMes (srts, teilchenIN(1:2), mediumATcollision, momentumLRF, teilchenOut(1:4), sigmaTot, sigmaElast)


    case (scenarioMesMes)
       HiEnergyFlag = .false.
       call MesMes (srts, teilchenIN(1:2), mediumATcollision, momentumLRF, teilchenOut(1:3), sigmaTot, sigmaElast)
  ...
  contains
    ...
    logical function DecideHiEnergy (srts, srts0, dsrts)
      ...
      if (.not. smoothHiEnergy) then
        DecideHiEnergy = (srts > srts0)
        return
      end if
    ...
    subroutine mesMes (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT, sigmaTot, sigmaElast)
      ...
      use IdTable, only: pion, kaon, kaonBar, kaonStar, kaonStarBar, rho, omegaMeson, phi, sigmaMeson, etaPrime, nmes
      use mesonWidthMedium, only: decayWidthMesonMedium, WidthMesonMedium
      use random, only: rn
      use mesonMeson_Xsections, only: sig_pipi2kkbar, kkbar_cross, kkbar_out, kstarkbar_cross, kstarkbar_out
      ...
      do ichannel=1,nDecay2bodyMeson
         if ((teilchenIn(1)%ID==Decay2BodyMeson(ichannel)%ID(1) .and. teilchenIn(2)%ID==Decay2BodyMeson(ichannel)%ID(2)) .or. &
             (teilchenIn(1)%ID==Decay2BodyMeson(ichannel)%ID(2) .and. teilchenIn(2)%ID==Decay2BodyMeson(ichannel)%ID(1))) then
            hasResonant = .true.
            exit
         endif
      enddo
      ...
      if (hasResonant) then
         ...
         do idRes = pion,pion+nmes-1
            ...
            do k=1,nDecays
               ...
               width_meson = decayWidthMesonMedium(idRes, resmass, izt, pauliFlag)        ! get the in-width
               gamtot = WidthMesonMedium(idRes, resmass, momentumLRF, mediumATcollision)  ! total in-medium width (incl. collisional width)
      ...
      if ((teilchenIn(1)%ID==pion .or. teilchenIn(1)%ID==rho) .and. &
         ...
         if (srts>srts0) then
            ...
            if (isofac>0.) then
               ...
               if (teilchenIn(1)%ID/=teilchenIn(2)%ID) sigbgt = sigbgt*2.
            endif
         endif
         ...
         call kkbar_cross(izt,srts,pinitial2,const,msigbg)
         ...
         call kstarkbar_cross(izt,srts,pinitial2,const,msigbg)
      ...
      case (2) ! === background ===
         ...
         if (teilchenIn(1)%ID>=pion .and. teilchenIn(1)%ID<=etaPrime .and. &
            ...
            call kstarkbar_out(izt,msigbg,sigbgt,teilchenOut)
...
!         call TRACEBACK("elastic mesmes not yet implememnted")
    ...
    !*************************************************************************
    subroutine barMes (srts, teilchenIN_Original, mediumATcollision, momentumLRF, teilchenOUT, sigmaTot, sigmaElast)
      ...
      if( isMeson(teilchenIN(1)%ID) ) then
         if(teilchenIn(1)%antiparticle) then ! If meson is an antiparticle then we first convert it to a particle
            write(*,*) 'There is a meson declared as Anti-Meson! Please tell Olli!', teilchenIn(1)%antiparticle, teilchenIn(1)%ID
         ...
         if(teilchenIn(2)%antiparticle) then ! If meson is an antiparticle then we first convert it to a particle
            write(*,*) 'There is a meson declared as Anti-Meson! Please tell Olli!', teilchenIn(2)%antiparticle, teilchenIn(2)%ID
      ...
      if (HiEnergyFlag) then
         If (antiBaryon) baryonID=-baryonID     ! Setting for "paramBarMesHE" (old BUU scheme = antiparticles have minus sign in ID)
         call paramBarMesHE(srts,mesonID,baryonID,mesonCharge,baryonCharge,mediumAtCollision,sigmaTot,sigmaElast)
         if (mesonID == omegaMeson .and. omega_K_factor>1.) sigmaTot = (sigmaTot-sigmaElast)*omega_K_factor + sigmaElast
         return
      end if
      ...
      Case(nucleon)
         Select Case(mesonID)
         Case(pion)
            call pionNuc (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:4), sigmaTot, sigmaElast, plotIT)
         Case(omegaMeson)
            call omegaNuc (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, &
                           useHiEnergy, HiEnergyThresholdBarMes, plotIT, omega_K_factor)
         Case(rho)
            call rhoNuc (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, &
                         useHiEnergy, HiEnergyThresholdBarMes, plotIT)
         Case(sigmaMeson)
            call sigmaNuc (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, &
                           useHiEnergy, HiEnergyThresholdBarMes, plotIT)
         Case(eta)
            call etaNuc (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, &
                         useHiEnergy, HiEnergyThresholdBarMes, plotIT)
         Case(phi)
         ...
         Case(Kaon)
            if (.not.antiBaryon) then
               ...
               call kaonBarNuc (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, plotIT)
            end if
         Case(KaonBar)
            if (.not.antiBaryon) then
               call kaonBarNuc (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, plotIT)
         ...
         Case(KaonStar)
            if (.not.antiBaryon) then
               ! Not implemented.
!!$          call KaonStarNuc(srts,teilchenIN,mediumATcollision,momentumLRF,teilchenOUT(1:3),sigmaTot,sigmaElast,plotIT)
            else
               call kaonBarNuc (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, plotIT)
            end if
         Case(KaonStarBar)
      ...
      Case(delta)
         Select Case(mesonID)
         Case(pion)
            call pionDelta (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, plotIT)
         Case(rho)
            call rhoDelta (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, &
                           useHiEnergy, HiEnergyThresholdBarMes, plotIT)
         Case(eta)
      ...
      Case(P11_1440)
         Select Case(mesonID)
         Case(pion)
            call pionP11_1440 (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, &
                               useHiEnergy, HiEnergyThresholdBarMes, plotIT)
         End Select
      Case(SigmaResonance)
         Select Case(mesonID)
         Case(pion)
            call mesonY (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, plotIT)
         Case(kaon)
      ...
      Case(Lambda)
         ...
         Case(pion,eta)
            call mesonY (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, plotIT)
         Case(kaon)
            call kaonLambda (srts, teilchenIN, mediumATcollision, momentumLRF, teilchenOUT(1:3), sigmaTot, sigmaElast, plotIT)
         Case(kaonBar)
...
!!$       Select Case(mesonID)
!!$       Case(pion)
!!$          call pionXi(srts,teilchenIN,mediumATcollision,momentumLRF,teilchenOUT(1:3),sigmaTot,sigmaElast,plotIT)
!!$       End Select
!!$
!!$    Case(Lambda_CPlus)
!!$       Select Case(mesonID)
!!$       Case(pion)
!!$          call pionLambda_CPlus(srts,teilchenIN,mediumATcollision,momentumLRF,teilchenOUT(1:3),sigmaTot,sigmaElast,plotIT)
!!$       End Select
!!$
!!$    Case(Xi_C)
!!$       Select Case(mesonID)
!!$       Case(pion)
!!$          call pionXi_c(srts,teilchenIN,mediumATcollision,momentumLRF,teilchenOUT(1:3),sigmaTot,sigmaElast,plotIT)
!!$       End Select
    ...
    ! sigmaTot, sigmaElast, sigmaCEX, sigmaAnni, sigmaLbar, sigmaSbar, sigmaXiBar, sigmaJPsi, PauliIncluded)
    !
    ! PURPOSE
    ...
    subroutine barBar (srts, teilchenIn, mediumATcollision, teilchenOut, &
      ...
      if (HiEnergyFlag) then
         ...
         ! no collisions of S=-3 or C=+1 resonances
         if(max(hadron(teilchenIN(1)%ID)%charm,hadron(teilchenIN(2)%ID)%charm).gt.0) return
         if(min(hadron(teilchenIN(1)%ID)%strangeness,hadron(teilchenIN(2)%ID)%strangeness).lt.-2) return
         ...
         If(teilchenIn(1)%antiParticle.neqv.teilchenIn(2)%antiParticle ) then
            ...
            sigmaTot   = sigtot_NN (srts, sum(teilchenIn(1:2)%charge))
         ...
         If( teilchenIN(1)%antiparticle .and. teilchenIN(2)%antiparticle ) then
            ...
            if (plotIT) then
               ...
               call XsectionBarBar (srts, teilchenIN, mediumATcollision, teilchenOut, sigmaTot, sigmaElast, pauliIncluded)
            end if
         end if
      end if
  ...
  !*************************************************************************
  !****f* master_2Body/sigtot_NN
  ! NAME
  ! real function sigtot_NN(srts, totalcharge)
  !
  ! PURPOSE
  ! This routine provides a parametrization of the total nucleon-nucleon
  ! cross section, consisting of different pieces for different energy regimes.
  !
  ! INPUTS
  ...
  real function sigtot_NN (srts, totalcharge)
    ...
    if (totalcharge == 1) then
      ! p+n
      if (p>2.245) then
         ! high-energy parametrization for p+n according to PDG
         sigtot_NN = 47.3 + 0.513*log(p)**2 - 4.27*log(p)
      else if (p>0.9) then
         ! Cugnon (this part is rather crude, needs improvement!)
         sigtot_NN = 24.2 + 8.9*p
      else
         ! elastic
         sigtot_NN = nukNuk_nukNuk (srts, (/mN,mN/), totalcharge)
      end if
    else
      ! p+p, n+n
      if (p>7.438) then
         ! high-energy parametrization for p+p according to PDG
         sigtot_NN = 48.0 + 0.522*log(p)**2 - 4.51*log(p)
      else if (p>1.5) then
         ! Cugnon
         sigtot_NN = 41. + 60.*(p-0.9)*exp(-1.2*p)
      else if (p>0.7) then
         ! Cugnon
         sigtot_NN = 23.5 + 24.6/(1.+exp((1.2-p)/0.1))
      else
         ! elastic
         sigtot_NN = nukNuk_nukNuk (srts, (/mN,mN/), totalcharge)
      end if
    end if
  end function sigtot_NN
  ...
  !
  ! NOTES
  ! This routine assumes a smooth transition (as for smoothHiEnergy=.true.).
