...
! PURPOSE
! Administrates the calculation for Xsections for strangeness production 
! from BB->BYK binary collisions. The properties of the final state channels 
! are calculated as well.
! NOTES
! * All isospin channels are included. Ref.: K. Tsushima et al., PRC59 (1999) 369.
! * Useful for reactions near and above threshold, not appropriate for 
!   sqrt(s) far below threshold (Xsextions does not fit well existing data & 
!   perturbative treatment of kaon production necessary).
!**********************************************************************************
module barBar_BarHypKaon
 ...
 contains
   ...
   ! OUTPUT
   ! * real, allocatable,dimension(:) :: sigma_BYK -- Xsection for each channel (mb)
   ! NOTES
   ! * This routine calculates also the properties of the finalstates, needed for 
   !   the construction of the Type(PreEvent) variable.
   ...
   function barBar_barBarMeson_strange (srts, teilchenIN) result (sigma_BYK)
     ...
     real,           dimension(1:18)            :: sigma_BYK      ! BB->BYK Xsection (in units of mb)
     ! Local variables
     ...
     nexit           = 0
     nexit0          = 0
     sigma_BYK(:)    = 0.0  !xsection
     IdsOut(:,:)     = 0    !ID's of 3-body final state
     ChargesOut(:,:) = 9999 !charges of 3-body final state
     !------------------------------------------------------------------------------
     ! (2) Only for nucleons,delta's,P11_1440's as incoming particles
     !------------------------------------------------------------------------------
     if ((teilchenIN(1)%ID > P11_1440).or.(teilchenIN(2)%ID > P11_1440)) return
     !------------------------------------------------------------------------------
     ! (3) Define charge states of incoming particles
     !     Convert variables related to charge states from GiBUU to RBUU sheme
     !------------------------------------------------------------------------------
    ...
    real function xs_param (s, j)
      ...
      !-----------------------------------------------------------------------------
      ! (0) The table with the threshold and cross section parameters for the 
      !     primary channels of kaon production from BB collisions:
      ...
      ! SOURCE
      real,dimension(1:Nch), save :: a = a0
      ! PURPOSE
      ! This array contains the "a" parameters (in fm) for the 30 primary channels,
      ! cf. Tsushima et al., PRC59 (1999) 369, table III.
      !************************************************************************
      real :: x
    ...
    contains 
      ...
      subroutine readInput
        ...
        do i=1,Nch
          if (abs(a(i)-a0(i))>1E-3) write(*,'(i3,3f8.3)') i, a(i), a0(i), a(i)/a0(i)
        end do
    ...
    Function Get_Charge(direction,species,iso_in) result (iso_out)
      ...
      use IdTable, only: nucleon,delta,P11_1440,Lambda,SigmaResonance,Kaon,lambda_1405,SigmaStar
    ...
    Integer Function Get_ID (species, iso)
      ...
      use IdTable, only: nucleon,Delta,Lambda,SigmaResonance,lambda_1405,SigmaStar
