...
module barBar_barBarMes
  ...
  public :: NN_NNpi_direct, sig_NNV, chooseCharge_NNpiomega, Np_NRplus_NNPion, sstatepi
...
contains
  ...
  function NN_NNpi_direct (srtfree, bg) result(Sigma_NNPion)
    ...
    if (srtfree > 2*mN + mPi) then
      sigmaAdd = sstatepi (srtFree, bg)                ! non-resonant background
  ...
  subroutine Np_NRplus_NNPion (srtS, background, ch, sigmaDelta, sigma1_2, sigma3_2)
    ...
    If (.not.(background.and.hadron(Delta)%propagated)) then       ! If Delta is not switched off
       If (p_cm > 1.0E-9) then
          call dimiIntegrated(dsdm_Integrated,srts)
          ! factor 1/3 because dimixsec contains the xsection for p p->n D++. And Clebsch(N p->N D+)/(p p->n D++) = (1/4) / (3/4) = 1/3
          sigmaDelta = dsdm_Integrated / (3.*p_cm)
       END IF
    end if
  ...
  real function sig_NNV (srts, ch)
    ...
    select case (ch)
    case(1)  ! omega
      srts0 = 2.*mN+hadron(omegaMeson)%mass
    case(2)  ! phi
      srts0 = 2.*mN+hadron(phi)%mass
    case(3)  ! pi-omega
      srts0 = 2.*mN+mPi+hadron(omegaMeson)%mass
    case default
      write(*,*) "bad channel in sig_NNV: ", ch
      stop
    end select
