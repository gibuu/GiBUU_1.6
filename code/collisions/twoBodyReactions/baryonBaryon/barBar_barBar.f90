...
module barBar_BarBar
  ...
  logical, parameter :: deltaN_densityDependence = .false.  ! Switch to switch off the density dependence of the NN <-> N Delta cross section
  real, parameter :: alphaD = 0.                            ! Parameter which controls the density dependence of the NN <-> N Delta cross section
...
contains
  ...
  real function NN_NDelta (srtFree,pInitial,idOut,isoIn,chargeIn,mediumAt)
    ...
    call dimiIntegrated(dsdm_Integrated,srtfree)
    NN_NDelta = dsdm_Integrated * iifac0(idOut,isoIn,ChargeIn) * 4./3./pinitial

    if (deltaN_densityDependence .and. mediumAt%useMedium) then
      NN_NDelta = NN_NDelta*(1.+alphaD*(mediumAt%densityProton+mediumAt%densityNeutron)/rhoNull)
    end if
  ...
  real function NDelta_NN(srtFree,pInitial,idOut,isoIn,chargeIn,mediumAt,massDelta)
    ...
    call dimiSigma(sigma,srtfree,massDelta)
    NDelta_NN = sigma * iifac0(idOut,isoIn,ChargeIn) * 8./3./pinitial

    if (deltaN_densityDependence .and. mediumAt%useMedium) then
      NDelta_NN = NDelta_NN*(1.+alphaD*(mediumAt%densityProton+mediumAt%densityNeutron)/rhoNull)
    end if
