...
c=================================================================
c This is a copy of the original PYTHIA-PYXTOT (v6.225)
c Changes due to: HERMES:
c - VINT(317): respect MSTP(20)=0
c
c make sure, that the original PYXTOT is not linked
C*********************************************************************
      ...
      ELSEIF(MINT(101).EQ.4.AND.MINT(102).EQ.1) THEN
        IF(MINT(107).EQ.2) THEN
          IF(MSTP(20).EQ.0) THEN
            VINT(317)=(PMHAD(2)**2/(PMHAD(2)**2+VINT(307)))**2.575
         else
            VINT(317)=(PMHAD(2)**2/(PMHAD(2)**2+VINT(307)))**2.0
          ENDIF
        ELSE
