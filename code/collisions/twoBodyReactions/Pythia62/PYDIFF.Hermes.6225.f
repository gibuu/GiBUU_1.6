...
c=================================================================
c This is a copy of the original PYTHIA-PYDIFF (v6.225)
c changes due to: HERMES
c
c Changing the angular distribution of diffractive rho decays
c (as measured by HERMES). In addition eps as in PYSIGH.HERMES.f
c
...
c... (measured by HERMES)

              r_c0 = 0.33
              r_c1 = 0.61
              R_rho=r_c0*(VINT(307)/P(N,5)**2)**r_c1
              BEAMAS=PYMASS(11)
              eps=1D0/(1D0+(VINT(309)**2*(1D0-2D0*BEAMAS**2/
     &            VINT(307)))/(2D0/(1D0+VINT(307)/VINT(309)**2/
     &            VINT(290)**2)*(1D0-VINT(309)-
     &            (VINT(307)/4D0/VINT(290)**2))))
