...
c=================================================================
c This is a copy of the original PYTHIA-PYRAND (v6.225)
c output of "auxiliary warnings" (FORMAT 5400, 5500, 5600, 5700) 
c commented out
c
c make sure, that the original PYRAND is not linked
c
c additional modifications:
c 1) counter of "goto 100" loops stops after 10000 loops
c 2) PYSAVE(4,...) redone, if GVMD event for W < 2.218+VINT(104)
c 3) comment at line 578: probably a bug: 
c    PMM is used instead of sqrt(PMM)
C*********************************************************************
...
C...User process initialization and event commonblocks.
      ...
      COMMON/QEPRUP/IDBMUP(2),EBMUP(2),PDFGUP(2),PDFSUP(2),
     ...
     &LPRUP(MAXPUP)
      ...
      COMMON/QEPEUP/NUP,IDPRUP,XWGTUP,SCALUP,AQEDUP,AQCDUP,IDUP(MAXNUP),
...
!         write(*,*) 'WARNING: PYRAND-Loop = ',LOOP
...
C...For gamma-p or gamma-gamma first pick between alternatives.
 101    IGA=0
        ...
        if ((IGA.eq.3).and. VINT(1).lt. ! [KG] no GVMD below W_min
...
C...Determine maximum possible t range and coefficient of generation.
     ...
     &  (SQM1*SQM4-SQM2*SQM3)/SH
        THL=-0.5D0*(THA+THB)
        THU=THC/THL
        THRND=EXP(MIN(7D2,MAX(-50D0,BMN*(THL-THU))))-1D0
 
C...Select diffractive mass/masses according to dm^2/m^2.
        LOOP3=0
  260   LOOP3=LOOP3+1

        if (LOOP3.gt.1000) then
      ...
      ELSEIF(MSTP(123).EQ.1) THEN
        IF(VIOL.GT.VINT(108)) THEN
          ...
          IF(VIOL.GT.1.0001D0) THEN
            MINT(10)=1
c            WRITE(MSTU(11),5400) VIOL,NGEN(0,3)+1
c            IF(MSTP(122).GE.2) WRITE(MSTU(11),5100) ISUB,VINT(21),
c     &      VINT(22),VINT(23),VINT(26)
          ENDIF
        ENDIF
...
c          WRITE(MSTU(11),5400) VIOL,NGEN(0,3)+1
     ...
     &      VINT(22),VINT(23),VINT(26)
c            IF(ISUB.LE.9) THEN
c              WRITE(MSTU(11),5500) ISUB,XSEC(ISUB,1)
c            ELSEIF(ISUB.LE.99) THEN
c              WRITE(MSTU(11),5600) ISUB,XSEC(ISUB,1)
c            ELSE
c              WRITE(MSTU(11),5700) ISUB,XSEC(ISUB,1)
c            ENDIF
          ENDIF
          VINT(108)=1D0
        ENDIF
