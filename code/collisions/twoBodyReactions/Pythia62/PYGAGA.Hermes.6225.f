...
c=================================================================
c This is a copy of the original PYTHIA-PYGAGA (v6.225)
c Changes due to: K.Gallmeister
c instead of random phi-selection for the photon, phi is fixed to 0.
c
c Changes due to: HERMES:
c new VINT(319)(=WTGAGA) according to better epsilon calculation
c
c make sure, that the original PYGAGA is not linked
C*********************************************************************
...
C...description of the lepton -> lepton + photon branching.
      ...
      SUBROUTINE PYGAGA_hermes(IGAGA,WTGAGA)
...
c              WTGAGA=WTGAGA*(1D0-XY)
              IF((MINT(11).EQ.22).and.
     &             (MINT(12).EQ.2212.or.MINT(12).EQ.2112)) THEN
               pmass=PYMASS(2212)
               WTGAGA=WTGAGA*(1D0/(1D0+(Q2(I)/XY**2/
     &                VINT(290)**2))*
     &               (1D0-XY-(Q2(I)/4D0/VINT(290)**2)))/
     &               Q2(I)/XY**2/VINT(290)*
     &               (VINT(290)*XY-Q2(I)/2D0/pmass)*XY*Q2(I)
              ELSE
...
!              write(*,*) 'WTGAGA:',Q2(I),XY,VINT(290)

              WTGAGA=WTGAGA*(0.5D0*((VINT(290)*XY-Q2(I)/2D0/
     &               pmass)/Q2(I)/XY**2/VINT(290)*
     &               (XY**2*(1D0-(2D0*emass**2/Q2(I)))+
     &               (2D0/(1D0+(Q2(I)/XY**2/VINT(290)**2)))*
     &               (1D0-XY-(Q2(I)/4D0/VINT(290)**2))))*
     &                XY*Q2(I))
            ELSE
