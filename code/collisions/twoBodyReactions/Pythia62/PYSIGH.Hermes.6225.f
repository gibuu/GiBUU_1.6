...
c=================================================================
c This is a copy of the original PYTHIA-PYSIGH (v6.225)
c Changes due to: HERMES:
c - respect MSTP(17)=6
c - respect better epsilon
c
c make sure, that the original PYSIGH is not linked
C*********************************************************************
...
C...the conversion factor from GeV^-2 to mb
      ...
      SUBROUTINE PYSIGH_hermes(NCHN,SIGS)
...
C...Commonblocks
     ...
     &SFMIX(16,4),ZMIXI(4,4),UMIXI(2,2),VMIXI(2,2)
      double precision ZMIX,UMIX,VMIX,SMZ,SMW,SFMIX,ZMIXI,UMIXI,VMIXI
      SAVE /PYSSMT/
     ...
     &SHR,SQPTH,TAUP,BE34,CTH,SQMZ,SQMW,GMMZ,GMMW,
     &AEM,AS,XW,XW1,XWC,XWV,POLL,POLR,POLLL,POLRR
      integer
     $     ISUB,ISUBSV,MMIN1,MMAX1,MMIN2,MMAX2,MMINA,MMAXA,
     $     KFAC
      double precision
     $     COMFAC,FACK,FACA,SH,TH,UH,SH2,TH2,UH2,SQM3,SQM4,
     &     SHR,SQPTH,TAUP,BE34,CTH,SQMZ,SQMW,GMMZ,GMMW,
     &     AEM,AS,XW,XW1,XWC,XWV,POLL,POLR,POLLL,POLRR
      SAVE /PYSGCM/

C...Local arrays and complex variables
      DIMENSION X(2),XPQ(-25:25)
...
C ........Hermes version of R_VMD
            ELSEIF(MSTP(17).EQ.6) THEN
              PMVIRT=PMAS(PYCOMP(113),1)
              REDUCE=(Q2GA/PMVIRT**2)**0.61

            ENDIF
            BEAMAS=PYMASS(11)
            IF(VINT(302+ISDE).GT.0D0) BEAMAS=VINT(302+ISDE)
c            FRACLT=1D0/(1D0+XY**2/2D0/(1D0-XY)*
c     &      (1D0-2D0*BEAMAS**2/Q2GA))
