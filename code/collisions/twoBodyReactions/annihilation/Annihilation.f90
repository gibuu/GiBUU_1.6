...
contains
  ...
  !*************************************************************************
    ...
    real, dimension(6,3)  :: pn      ! first index : particle, second index : momentum index
...
!*** and we have to rely on string model:
    ...
    case(1)  ! String-based model
        ...
        if( srtS .gt. 4.*mPi ) then   ! Use HiEnergy package:
              ...
              call momenta_in_nBodyPS(srtS,finalState(1:maxID)%mass,pn(1:maxID,1:3))
              do i=1,maxID
                finalState(i)%momentum(1:3)=pn(i,1:3)
                finalState(i)%momentum(0)=sqrt( finalState(i)%mass**2 &
                                        &      +dot_product(pn(i,1:3),pn(i,1:3)) )
              end do
