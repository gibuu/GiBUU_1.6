...
!!$    call WriteParticle(6,1,2,inPart(2))
    ...
    !...no collision for baryonic resonances Xi,Omega,Sigma,... for sqrts<3:

    if (sqrtS.lt.3.0) then
       if (((inPart(1)%ID.lt.100).and.(inPart(1)%ID.gt.52)) &
            .or.((inPart(2)%ID.lt.100).and.(inPart(2)%ID.gt.52))) then
          write(*,*) 'DoColl_Fritiof: not for Xi,Omega,Sigma,... and sqrts<3!'
          return
       endif
    endif
    ...
    call ConvertInPartFritiof(ID1,ID1c)
    call ConvertInPartFritiof(ID2,ID2c)
