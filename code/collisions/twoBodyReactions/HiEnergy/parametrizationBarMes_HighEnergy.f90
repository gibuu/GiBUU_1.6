...
contains
  ...
  ! * Here we call corresponings single standing routines and perform 
  !   some modifications in order to guarantee a smoother transition
  !   from Hi to Lo energy [internal name: "flanschen" !]
  ...
  subroutine paramBarMesHE(srts,idM,idB,izM,izB, media, sigma,sigmael)
    ...
    integer, dimension(103:109), parameter :: iivec = &
         & (/1,0,2,0,3,0,4/) ! rho,omega,phi or J/psi?
    ...
    ! This is because of problems in Ariadne:
    if(abs(idB).eq.omegaResonance) return

    ! select whether rho is rho0 or not...
    iidM = idM
    if ((iidM.eq.rho).and.(izM.ne.0)) iidM = 0
    ...
    case(rho,omegaMeson,phi,JPsi) !==== Vector mesons ====
       ...
       if(abs(idB).eq.nucleon) then
          ...
          select case (ivec)
          case(1) ! == rho0 ==
             if (srts.le.5.0) sigmael = min(3.15,sigmael)
          case(2) ! == omega ==
    ...
    case(kaon:kaonStarBar) !==== Kaons ====
       ...
       case(kaon,kaonStar)
          izM2 = -2
          if (izB2.le.0.and.izM.eq.1 .or. izB2.gt.0.and.izM.eq.0) izM2 = -1
          
          if (abs(idB).eq.nucleon .and. idM.eq.kaon) then
             if(srts.le.2.614 .and. idB.gt.0) then
                E=(srts**2-mK**2-mN**2)/(2.*mN)
                plab=sqrt(E**2-mK**2)
                if(izM+izB.eq.1) then
                   sigmael=sigelkp(plab,2)
                else
                   sigmael=sigelkp(plab,1)
                end if
             else if(srts.le.4.8) then
       ...
       case(kaonBar,kaonStarBar)
          ...
          if (abs(idB).eq.nucleon .and. idM.eq.kaonBar) then
             if(izM2*sign(1,idB).eq.2.and.srts.le.2.85) then
                sigmael=20*exp(-srts+1.32)
             else if(izM2*sign(1,idB).eq.1.and.srts.le.4.469)  then
...
!          sigma=20*exp(-srts+2)+14.5
       ...
       if (idM.eq.kaonStarBar.and.izM2.eq.2.and.srts.le.4.4) then
          sigma=20*(1-exp(-2*(srts-2.06)))+3
          
       else if (idM.eq.kaonStarBar.and.izM2.eq.1.and.srts.le.5.95) then
          sigma=20*(1-exp(-2*(srts-2.18)))
    ...
    case DEFAULT !==== all other mesons/ nonstrange baryons====
       ...
       if (abs(idB).eq.1.and.idM.eq.pion) then 
    ...
    if (sigmael.lt.1e-8) then
       sigmael = 0.039*sigma**(3./2.)
       
       select case(idM)
       case(eta)
          if (srts.le.3.8) sigmael = 50*exp(-2.5*srts+4.6)+5.0

       case(rho)
          izM2=min(max((2*abs(izB)-1)*izM,-1),1)*sign(1,idB)
          select case(izM2)
          case(1)
             if (srts.le.3.3) sigmael=min(5.37,sigmael)
          case(-1)
             if (srts.le.6.06) sigmael=1.0-exp(-(srts-2.5))+4.0
          end select
       end select
    ...
    if(abs(idB).lt.Lambda) then
       xs=0.
       xc=0.
    else if(abs(idB).lt.Xi) then
       xs=1./3.
       xc=0.
    else if(abs(idB).lt.omegaResonance) then
       xs=2./3.
       xc=0.
    else if(abs(idB).lt.Lambda_CPlus) then
       xs=1.
       xc=0.
    else
       xc=1./3.
    ...
    if (sigma.lt.sigmael) then
       if (DoPr(1)) write(*,'(A,f8.4,4i5,2f12.5)') 'PROBLEMS in paramBarMesHE: sigma<sigmael: ',&
            & srts,idM,izM,idB,izB,sigma,sigmael
       sigmael=sigma
    end if
  ...
  subroutine paramBarMesHE_kaon(srts,sigma,sigel)
    ...
    integer i,j,ich
    real a(4,5),plab,a2(4,5)


    ! Parameter from Falter PHD Table 4.4, with Typo for K+ n, corrected from -1.3 to -0.89 

    !*total
    data((a(i,j),j=1,5),i=1,4)   / 18.1,  0., 0., 0.26, -1.,        &
         &          18.7,  0., 0., 0.21, -0.89,   &
         &          25.2,  0., 0., 0.38, -2.9,     &
         &           32.1,  0., 0., 0.66, -5.6/
    !*elastic
    ...
    ich=1    !==== K+ p
    sigma(-2)=min(a(ich,1)+a(ich,2)*plab**a(ich,3)+         a(ich,4)*(log(plab))**2+a(ich,5)*log(plab),20.)
    sigel(-2)=min(a2(ich,1)+a2(ich,2)*plab**a2(ich,3)+     a2(ich,4)*(log(plab))**2+a2(ich,5)*log(plab),sigma(-2))

    ich=2    !==== K+ n
    sigma(-1)=min(a(ich,1)+a(ich,2)*plab**a(ich,3)+a(ich,4)*         (log(plab))**2+a(ich,5)*log(plab),20.)
    sigel(-1)=min(a2(ich,1)+a2(ich,2)*plab**a2(ich,3)+a2(ich,4)*   (log(plab))**2+a2(ich,5)*log(plab),sigma(-1))

    ich=3    !==== K- n
    sigma(1)=min(a(ich,1)+a(ich,2)*plab**a(ich,3)+a(ich,4)*         (log(plab))**2+a(ich,5)*log(plab),25.)
    sigel(1)=min(a2(ich,1)+a2(ich,2)*plab**a2(ich,3)+a2(ich,4)*   (log(plab))**2+a2(ich,5)*log(plab),sigma(1))

    ich=4    !==== K- p
    sigma(2)=min(a(ich,1)+a(ich,2)*plab**a(ich,3)+a(ich,4)*         (log(plab))**2+a(ich,5)*log(plab),30.)
    sigel(2)=min(a2(ich,1)+a2(ich,2)*plab**a2(ich,3)+a2(ich,4)*   (log(plab))**2+a2(ich,5)*log(plab),sigma(2))
