...
module RMF
  ...
  ! * 1 --- NL1 from G.A. Lalazissis et al., PRC 55, 540 (1997)
  !   (K=211.29 MeV, m^*/m=0.57)
  ! * 2 --- NL3 from G.A. Lalazissis et al.,
  !   (K=271.76 MeV, m^*/m=0.60)
  ! * 3 --- NL2 set from A. Lang et al., NPA 541, 507 (1992)
  !   (K=210 MeV, m^*/m=0.83)
  ! * 4 --- NLZ2 set from M. Bender et al., PRC 60, 34304 (1999)
  !   (K=172 MeV, m^*/m=0.583)
  ! * 5 --- NL3* set from G.A. Lalazissis, private communication,
  !   (K=258.28 MeV, m^*/m=0.594)
  ! * 6 --- Same as N_set=3, but including the rho-meson.
  ! * 7 --- NL1 set from S.J. Lee et al., PRL 57, 2916 (1986)
  !   (K=212 MeV, m^*/m=0.57)
  ! * 8 --- NL2 set from S.J. Lee et al., PRL 57, 2916 (1986)
  !   (K=399 MeV, m^*/m=0.67)
  !*************************************************************************
  ...
  real, save :: m_nucleon       ! -- nucleon mass (GeV),
...
contains
  ...
  subroutine init
      ...
      ! * fact_kaon
      !*************************************************************************
      NAMELIST /RMF_input/ RMF_flag, grad_flag, lorentz_flag, fourMomDen_flag, kaonpot_flag, &
                           N_set, fact_pbar, fact_hyp, fact_antihyp, fact_Xi, fact_antiXi, fact_Kaon
      ...
      if (RMF_flag) then
         ...
         case (6)
            !Same as N_set=3, including the rho-meson.
            m_nucleon=0.938   ! -- nucleon mass (GeV),
            m_sigma=0.5505 ! -- sigma-meson mass (GeV),
            m_omega=0.7833  ! -- omega-meson mass (GeV),   
            m_rho=0.763000    ! -- rho-meson mass (GeV),
  ...
  ! The density rho must be in the interval 
  ! from 0 up to 12*rhoNull.
  !***************************************************************************
  real function fshift(rho)
    ...
    real, parameter :: A(1:8) = (/ 1.03099, 0.783659, 0.201221, 1.01769, 0.75293, 0.201221, 1.00994, 0.599025 /)
    real, parameter :: B(1:8) = (/ 1.52439, 1.41926,  0.888537, 1.57845, 1.63993, 0.888537, 1.67593, 1.16527  /)
