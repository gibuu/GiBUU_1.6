...
module baryonWidth
  ...
  ! public subroutines ::: 
  PUBLIC :: partialWidthBaryon, fullWidthBaryon, decayWidthBaryon, baryonWidth_gammaN
  PUBLIC :: GetMaxQ, cleanUp
...
contains
  ...
  ! PURPOSE
  ! This function calculates the mass-dependent partial width for a specific decay channel of a
  ! baryon resonance.
  ! INPUTS
  ...
  ! * real, OPTIONAL     :: mesonMass, baryonMass --
  ...
  real function partialwidthBaryon (ID, mass, inWidth, mesonID, baryonID, mesonMass, baryonMass)
    ...
    use baryonWidthVacuum, only: interactionRadius,getAngularMomentum,srts_srt_switch
    ...
    real  :: Width,partialVacuumWidth,momentum
    integer :: Drehimpuls, massIndex, i, dId
    logical :: lDummy
    ...
    If (mass<=hadron(ID)%minmass) then
       ...
       if (InWidth) then  !in width
          ...
          do i=1,nDecays
             ...
             If ((Decay2BodyBaryon(dId)%ID(1)==mesonID) .and. (Decay2BodyBaryon(dId)%ID(2)==baryonID)) then
                If (Decay2BodyBaryon(dId)%stable(1) .and. Decay2BodyBaryon(dId)%stable(2)) then
                   ...
                   DrehImpuls=getAngularMomentum(i,ID)
                   !hier fehlen die IsoSpion-Clebsche, ansonsten Formel 2.77 aus Effenbergers Dr.-Arbeit:
                   if (srts_srt_switch) then
                      width = width + partialVacuumWidth * BlattWeisskopf(momentum*interactionRadius,DrehImpuls)**2 * momentum &
          ...
          do i=1,nDecays
             ...
             if (dId<=0) cycle !  not 2Body
             if ((Decay2BodyBaryon(dId)%ID(1)==mesonID) .and. (Decay2BodyBaryon(dId)%ID(2)==baryonID)) then
                width = width + gammaField(ID,massIndex)%gammaTotal * gammaField(ID,massIndex)%ratio(i)
             end if
          end do
       end if

       partialwidthBaryon=width
    else
