...
module mesonWidthVacuum
  ...
  Public :: vacuumWidth, getAngularMomentum, dileptonWidth
...
contains
  ...
  !************************************************************************
  !****f* mesonWidthVacuum/getAngularMomentum
  ! NAME
  ! integer function getAngularMomentum(DecayChannel,partID)
  ! INPUTS
  ! * integer :: DecayChannel -- The number of the 2-body-decay-channel of
  !   the meson according to module decayChannels
  ...
  integer function getAngularMomentum(DecayChannel,partID)
    ...
    Case default
       ...
       call TRACEBACK('Not yet implemented in getAngularMomentum for mesons')
    End select
  ...
  contains
    ...
    real function gpipi(s)
      ...
      if (sqrt(s)>2.*mPi) then
         gpipi=hadron(omegaMeson)%width*rpipi*(qcm(sqrt(s),mPi,mPi)/qcm(mome,mPi,mPi))**3
      else
         gpipi=0.
      end if
    end function gpipi
    ...
    real function gamrho(s)
      ...
      real,intent(in)::s
      if (s>(2.*mPi)**2) then
         gamrho=1./(6.*pi)*frho**2*qcm(sqrt(s),mPi,mPi)**3/s
      else
         gamrho=0.
      end if
    end function gamrho
    ...
    real function qcm(srts,m1,m2)
      ! cms 3-momentum for decay of resonance with mass srts
      real,intent(in)::srts,m1,m2
      real::q2
      q2=(srts**2-(m1+m2)**2)*(srts**2-(m1-m2)**2)/4./srts**2
      if(q2>0.) then
         qcm=sqrt(q2)
      else
         qcm=0.
