...
!***********************
  ...
  do id=1,nbar
     ...
     do i=1,40  
        ...
        If(Abs(sum-FullWidthBaryon(id,mass)).gt.1E-5) then 
...
!!$
  ...
  do id=1,nbar
     ...
     do i=1,40  
        ...
        If(Abs(sum-widthBaryonMedium(id,mass,mom,MediumAtPosition)).gt.1E-5) then 
           Print *, "Sum of partial width not equal full width"
           Print *, "Resonance=",id
           Print *, "Mass=",mass
           Print *, sum, FullWidthBaryon(id,mass)
     ...
     do i=1,50  
       ...
       do mesonID=pion,dsStar_Minus
           do baryonID=nucleon,Omega_c
              ...
              If (partialwidthBaryon(ID,mass,.true.,mesonID,baryonID).gt.0.and.index.lt.size(feld)) then
                 index=index+1
   ...
   do id=1,nbar
      ...
      do i=1,50  
         ...
         do mesonID=pion,dsStar_Minus
            do baryonID=nucleon,Omega_c
               sum=sum+partialwidthBaryon(ID,mass,.false.,mesonID,baryonID)
               If (partialwidthBaryon(ID,mass,.false.,mesonID,baryonID).gt.0.and.index.lt.size(feld)) then
                  index=index+1
