...
module baryonWidthVacuum
  ...
  PUBLIC :: vacuumWidth, getAngularMomentum, getParameters, markusPostFormFactor
...
contains
  ...
  !************************************************************************
  !****if* baryonWidthVacuum/markusPostFormfactor
  ! NAME
  ! real function markusPostFormfactor(mass,pole,sNull,lambda)
  ! PURPOSE
  ...
  real function markusPostFormfactor (mass, pole, sNull, lambda)
    ...
    real, intent(in) :: sNull ! Decay threshold

    if (use_cutOff) then
       markusPostFormfactor=(lambda**4+1./4.*(sNull-pole**2)**2)/(lambda**4+(mass**2-1./2.*(sNull+pole**2))**2)
    else
       markusPostFormfactor=1.
    end if
...
!   end function monopoleFormfactor
  ...
  function vacuumWidth (mass, partID, ratio, rho_AB_atPole) result (gammaTotal)
    ...
    do k=1,nDecays
       ...
       L=getAngularMomentum(dID,partID)
  ...
  real function rho_AB_Integrand (mu)
    ...
    case(SigmaStar) ! Sigma* -> pion + Lambda
       ! this sigmaStar has mass mu, so we need gammaUnstable(mu)
       ! The qcm momentum of pion and lambda in the  restframe of sigmaStar are at the real mass "mu" of the sigmaStar:
       qcmUnstable_mu=pcm(mu,hadron(lambda)%mass,mPi)

       ! And at the sigmaStar's polemass:
  ...
  !************************************************************************
  !****f* baryonWidthVacuum/getAngularMomentum
  ! NAME
  ! integer function getAngularMomentum(DecayChannel,partID) 
  ! PURPOSE
  ! Return the angular momentum of the outgoing particles of a specific decay channel
  ! INPUTS
  ! * integer :: DecayChannel -- The number of the decay channel according to module decayChannels
  ...
  integer function getAngularMomentum(DecayChannel,partID) 
    ...
    integer,intent(in) :: DecayChannel
    integer,intent(in) :: partID
    ...
    Case(42) 
       if(partID.eq.lambda_2100) then
          L=2   ! Lambda(2100) --> omega Lambda
       else if(partID.eq.lambda_2110) then
    ...
    If(Debug) then
       If (L.gt.4) then
          Print  *,"L>4",L, "in getAngularMomentum"
          Print *, decayChannel,partID
       End if
    End if
    getAngularMomentum=L
  end function getAngularMomentum
