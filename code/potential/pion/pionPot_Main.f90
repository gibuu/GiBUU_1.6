...
contains
  ...
  !*****************************************************************************
  !****s* pionPot/pionPot_Main
  ! PURPOSE
  ! determine scalar potential for pion, to use in mesonpotential
  ! Potential is defined as oth component of a vectorpotential in the LRF
  ! routine is called by mesonpotential.
  ! The potential should not be used above 250 MeV momentum.
  !*****************************************************************************
  function pionPot_Main(masse,momentum,rhoProton,rhoNeutron,charge,potentialSwitch)
    ...
    real, dimension(1:3)  :: max_momentum
    real, intent(in)    :: masse
    integer,intent(in) :: potentialSwitch
    ...
    Case(1)
       ...
       call pionPot_Oset(momentum,rhoProton,rhoNeutron,charge,.true.,VOpt,gamma)
    Case(2)
       !Potential in Hamiltonian H=SQRT(p**2+m**2)+V(Nuk)+V(Coulomb)
       call pionPot_Kapusta(absMomentum,rho,VOpt)
    Case(3)
       If(absMomentum.gt.0.25) then
          !          write(*,*) 'Problem in file pionPot_Main.f90 subroutine pionPot_Main'
          !            write(*,*) 'Problem with pionPotential. Momentum too high:' ,absMomentum
       ...
       call pionPot_DeltaHole(momentum,rhoProton,rhoNeutron,masse,Vopt)
    Case(4)
       If(absMomentum.gt.0.25) then
          !            write(*,*) 'Problem in file pionPot_Main.f90 subroutine pionPot_Main'
          !              write(*,*) 'Problem with pionPotential. Momentum too high:' ,absMomentum
          !              write(*,*) 'If you want to propagate pions with such high momentum, then you need to switch off their potential!'
          !          pionPot_Main=0.
          max_momentum=(/0.25,0.,0./)
          call pionPot_DeltaHole(max_momentum,rhoProton,rhoNeutron,masse,Vopt)
          !
          !              write(*,*) 'Critical error! STOP'
          !             stop
       else
          call pionPot_Smooth(momentum,rhoProton,rhoNeutron,masse,charge,VOpt)
       end if
    Case default                     
       Vopt=0.
    End Select
  ...
  !*****************************************************************************
  subroutine pionPot_DeltaHole(p,rhop,rhon,masse,Vopt)
  ...
  end subroutine pionPot_DeltaHole


  !*****************************************************************************
  !****s* pionPot/pionPot_Kapusta
  ! PURPOSE
  ...
  !*****************************************************************************
  subroutine pionPot_Kapusta(k,rho,VOpt)
    ...
    real  :: omega,omegaNeu,mNull,kNull
    real  :: x
    ...
    Vopt=omegaNeu-omega
  end subroutine pionPot_Kapusta


  !*****************************************************************************
  !****s* pionPot/pionPot_Smooth
  ! PURPOSE
  ! Berechnet das potential des pions
  ! lineare extrapolation zwischen oset-potential um Ekin=0 und simplen DeltaHole-Potential
  ! Gueltigkeit: bis 130 MeV Ekin
  ! INPUTS
  ! * real   ::  rhop,rhon !proton and neutron density
  ...
  ! * real, intent(out)   :: Vopt !in GEV
  !*****************************************************************************
  subroutine pionPot_Smooth(momentum,rhop,rhon,masse,charge,VOpt)
    ...
    if (rhop+rhon.lt.0.001) then
       Vopt=0
       return
    end if

    If (qtot.lt.unten) then
       call  pionPot_Oset(momentum,rhop,rhon,charge,Deltaflag,VOpt,Decay)
    else if (qtot.gt.oben) then
       call pionPot_DeltaHole(momentum,rhop,rhon,masse,Vopt)
    else
       ...
       Do k=0,3
          ...
          mom(2)=0
          mom(3)=0
          call pionPot_oset(mom,rhop,rhon,charge,Deltaflag,VOpt,Decay)
          wert(k)=Vopt
       End do
       u=wert(0)
       ...
       Do k=0,3
          ...
          mom(2)=0
          mom(3)=0
          call pionPot_DeltaHole(mom,rhop,rhon,masse,Vopt)
          wert(k)=Vopt
       End do
       o=wert(0)
  ...
  Contains
    ...
    Function splineFunc(u,o,uPrime,oPrime,x) Result (Ergebnis)
      Real u,o,uPrime,oPrime,Ergebnis
      Real x
      Real a,b,c,d
      !Ableitung stetig
    ...
    Function splineFunc2(u,o,uPrime,oPrime,uDo,oDo,x) Result(Ergebnis)
      Real u,o,uPrime,oPrime,Ergebnis,uDo,oDo
      Real x
      Real a,b,c,d,e,f
  ...
  end subroutine pionPot_Smooth


  !*****************************************************************************
  !****s* pionPot/pionPot_oset
  ! PURPOSE
  ! evaluates real part of the pion selfenergy and pion decay width
  ! according to (NPA 554 pages 554 and following) 
  ! Oset, Salcedo et al.
  ! All calculations are done in MEV and then converted to GEV in the end!
  !
  ! NOTES
  ! This Routine calls two subroutines, which evaluate s-Wave and P-Wave of the Selfenergy.
  ! These Subroutines evaluate the full selfenergy.
  !*****************************************************************************
  subroutine pionPot_Oset(momentum,rhop,rhon,charge,Deltaflag,VOpt,Decay)
    ...
    !output
    real, intent(out)               :: VOpt     !Potential
    real, intent(out)               :: Decay    !Decay width
    ...
    If (Abs(charge+1).le.0.01) then           !Pi Minus
       ...
       Decay=(ImPwave+ImSwave)/E/(-1000.)
       !VOpt=Real(PwavePart)/2./E/1000.
       !Decay=(ImPwave)/E/(-1000.)  

    else if (Abs(charge-1).le.0.01) then      !Pi Plus
    ...
    else if (Abs(charge-0).le.0.01) then      !Pi Zero
       call swave(ImSwave,SwavePart)     !Evaluates S-wave
       call pwave(ImPwave,PwavePart)     !Evaluates P-Wave
       VOpt=Real(PwavePart+SwavePart)
       Decay=(ImPwave+ImSwave)/E
       ...
       Decay=((Decay+(ImPwave+ImSwave)/E)/2.)/(-1000.)
    ...
    ! RE[Pot] stetig gegen Null gehen lassen an Grenze des G�ltigkeitsbereichs
    if (bigFlag) then
