...
contains
  ...
  logical function deuteriumPL_inUse()
    ...
    deuteriumPL_inUse = (eventtype/=HeavyIon) .and. Allocated(deuterium_pointerList)
    ...
    if (allocated(deuterium_pointerList)) then
       ...
       do i=lbound(realP,dim=1),ubound(realP,dim=1)
          ...
          do j=lbound(realP,dim=2),ubound(realP,dim=2)
             ...
             case default
               write(*,*) 'ERROR: More than two nucleons in deuterium ! STOP'
               call traceback()
             end select
          end do
          if (numNucs<2) then
             write(*,*) 'ERROR: Less than two nucleons in deuterium ! STOP', numNucs
             stop
          end if
       end do
