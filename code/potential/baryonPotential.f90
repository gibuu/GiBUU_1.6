...
module baryonPotentialModule
  ...
  integer,parameter :: deuterium_EQS=7


  !*************************************************************************
  ! Parameters of potential:
  ! Units: alpha [MeV], beta [MeV], c [MeV], lambda [1/fm], eta [fm^4]
  real, parameter,dimension(1:11):: alpha  = &
       (/ -108.619,  -9.972, -286.999930,  -124.26110, -29.253, 0., 0., 0., -22.900,   5.72 ,  56.7/) 
  real, parameter,dimension(1:11):: beta   = &
       (/  136.779,  38.032,  233.651700,    71.03007,  57.248, 0., 0., 0.,  33.900,  13.60 ,   4.75/) 
  real, parameter,dimension(1:11):: tau    = &
       (/    1.259,   2.404,    1.225707,     2.00108,   1.760, 0., 0., 0.,   1.820,   2.61 ,   3.69/)
  real, parameter,dimension(1:11):: lambda = &
       (/    2.130,   2.126,    0.000000,     0.00000,   2.130, 0., 0., 0.,   0.960,   0.807,   1.180/) 
  real, parameter,dimension(1:11):: c      = &
       (/  -63.601, -63.601,    0.000000,     0.00000, -63.516, 0., 0., 0.,-102.000,-144.000,-145.000/)
  real, parameter,dimension(1:11):: eta    = &
       (/    0.400,   0.300,    0.400000,     0.27000,   0.360, 0., 0., 0.,   0.   ,   0.   ,   0./)
  !*************************************************************************
  ...
  !*************************************************************************
  !****g* baryonPotentialModule/symmetriePotFlag
  ! PURPOSE
  ...
  ! NOTES
  ! Can be set to different value in jobcard, namelist 'baryonPotential'.
  ! Needs to be turned on together with 'coulombFlag' (in namelist 'coulomb').
  !*************************************************************************
  ...
  PUBLIC :: getsymmetriePotFlag_baryon
  PUBLIC :: getPotentialEQSType
  PUBLIC :: deuterium_EQS
  PUBLIC :: rhoLaplace
...
contains
  ...
  subroutine init
    ...
    ! * dsymm
    !***********************************************************************
    NAMELIST /baryonPotential/ symmetriePotFlag, EQS_type, DeltaPot, HypPot, &
                               noPerturbativePotential, &
    ...
    write(*,*) ' SymmetriePotFlag  = ',symmetriePotFlag
    ...
    if( getYukawaFlag() .and. SurfacePotFlag) then
       call TRACEBACK('yukawa and gradient contributions can not be done together !!!!')
    end if

    target => getTarget()
    if ( (symmetriePotFlag.neqv.getCoulombFlag()) .and.  ( target%mass.gt.3  .and. eventType.gt.0 )  ) then
       call TRACEBACK('Only switching symmetriePotFlag and Coulomb synchronously makes sense!!!')
    end if
  ...
  ! 
  ! NOTES
  ! Baryon potential is defined as 0th component of a vector potential in 
  ! the LRF.
  !*************************************************************************
  real function BaryonPotential (teilchen, med, positionNotSet, EQS_in)
    ...
    case(99)       ! Variable Skyrme
       ...
       if (symmetriePotFlag .and. teilchen%id==nucleon .and. .not.teilchen%antiparticle) &  ! add symmetry potential
         out = out + dsymm*(med%densityProton-med%densityNeutron)/rhoNull*(float(teilchen%charge)-0.5)*2.
    ...
    case(1:5,9:11)
       ...
       if (symmetriePotFlag .and. teilchen%id==nucleon .and. .not.teilchen%antiparticle) &  ! add symmetry potential
         skyrme = skyrme + dsymm*(med%densityProton-med%densityNeutron)/rhoNull*(float(teilchen%charge)-0.5)*2
...
!          out = out + momentumDependentPart(pAbs,c(EQS98MomDep),lambda(EQS98MomDep),med%density)
    ...
    Case default
       write(*,*) 'ERROR: Wrong EQS-Type in baryonPotential'
       write(*,*) 'EQS=', EQS
       call TRACEBACK()
...
!    LDApotential=(2.*b1*rho+7./3.*b2*rho**(4./3.)+8./3.*b3*rho**(5./3.)-2*a*rhoLaplace(teilchen%position,stepsize)-e0f*E0)*hbarc/1000.
  ...
  real function rearrangementPotential(teilchen,rhoBaryon,rhoProton,rhoNeutron)
    ...
    type(particle), intent(in) :: teilchen
    real, intent(in) :: rhoBaryon,rhoNeutron,rhoProton
    ...
    if (hadron(teilchen%id)%strangeness.lt.-1) return
    if (hadron(teilchen%id)%charm.ne.0) return
    ...
    case(1:5,9:11)
       ...
       skyrme_b=(alphaRenorm*(rhoBaryon/rhoNull) &
            & + 2./(tau(EQS_Type)+1.)*beta(EQS_type) &
            & *(rhoBaryon/rhoNull)**tau(EQS_Type))/1000.

       place=teilchen%position(1:3)
       skyrme_b=skyrme_b+yukpot(place)
       ...
       if(symmetriePotFlag.and.teilchen%id.eq.nucleon.and..not.teilchen%antiparticle) then
          rhoDiff = rhoProton-rhoNeutron
          rhoSum  = rhoProton+rhoNeutron
    ...
    case(98)
       if (teilchen%id.eq.nucleon.and..not.teilchen%antiparticle) then
          place=teilchen%position(1:3)
          sqrtR = sqrt(Dot_Product(place,place))
          ! the following is necessary, due to possible overflow from nint:
          if (sqrtR.ge.float(ubound(StorePotP,1))*StorePotDX) then
    ...
    case(1:2,5,9:11) ! momentum dependent
       pAbs=Sqrt(Dot_Product(Teilchen%momentum(1:3),Teilchen%momentum(1:3)))
       rearrangementPotential=rearrangementPotential      &
            & -momentumDependentPart(pAbs,c(EQS),lambda(EQS),rhoBaryon)/2
    end select

    If (hadron(teilchen%id)%strangeness.eq.0) then
      if(Abs(hadron(teilchen%ID)%Spin-1.5).lt.0.001) then !Delta resonance
         select case(deltaPot)
         case (1)
  ...
  subroutine HandPotentialToDensityStatic(nuc)
    ...
    do iLoop=1,nLoopReAdjust
       ...
       do i=0,nuc%MaxIndex
          ...
          if (EQS_type.eq.98) then ! iLoop=2,3,...
...
!    stop
  ...
  contains
    ...
    subroutine CalculateRearrangePot
      ...
      do i=nuc%MaxIndex-1,0,-1
         ...
         write(213,'(i5,10f12.5)') i,x,rB,StoreUbP(i)*1000,StoreUbN(i)*1000, &
  ...
  !*************************************************************************
  !****f* baryonPotentialModule/getsymmetriePotFlag_baryon
  ! NAME
  ...
  logical function getsymmetriePotFlag_baryon()
    if (initFlag) call init
    getsymmetriePotFlag_baryon=symmetriePotFlag
  end function getsymmetriePotFlag_baryon
