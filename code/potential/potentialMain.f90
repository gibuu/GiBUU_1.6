...
module potentialModule
  ...
  !
  ! PURPOSE
  ! Evaluates hadronic potential of particles in local restframe. It's considered to be the
  ! 0th component of a vector potential.
...
contains
  ...
  !*************************************************************************
  function trueEnergy(teilchenIn)
    use particleDefinition
    use densitymodule, only : densityAt, boostToLRF
    ...
    !output:
    real :: trueEnergy
    real :: rhoBaryon,rhoProton,rhoNeutron
    type(dichte) :: density
    ...
    if(isBaryon(Teilchen%Id)) then
       ...
       density=densityAt(place) !density in CF
       !Evaluate density in LRF :
       rhoBaryon=SQRT(density%baryon(0)**2-Dot_Product(density%baryon(1:3),density%baryon(1:3)))
       rhoProton=SQRT(density%proton(0)**2-Dot_Product(density%proton(1:3),density%proton(1:3)))
       rhoNeutron=SQRT(density%neutron(0)**2-Dot_Product(density%neutron(1:3),density%neutron(1:3)))
       call boostToLRF(teilchen,1)  !boost calculation frame to LRF
       teilchen%momentum(0)=teilchen%momentum(0) &
          &+    rearrangementPotential(teilchen,rhoBaryon,rhoProton,rhoNeutron)
       call boostToLRF(teilchen,2)  !boost LRF to Calculation frame
