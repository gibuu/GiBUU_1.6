...
contains
    ...
    If (UseHadronic) then !with hadronic potentials
          ...
          do i=1,3 !loop over x,y,z
             ...
             do j=-DerivativeType,DerivativeType
                ...
                if(treatParticleOffShell(teilchen%Id,teilchen%offshellparameter)) then
                   energy(i,j)=hamiltonFunc_offshell(teilchen,outOFBounds)
                   if(outOfBounds) then
                   ...
                   energy(i,j)=teilchen%momentum(0)
                end if
             end do
       ...
       if(treatParticleOffShell(teilchenIn%Id,teilchenIn%offshellparameter).and.dh_dp0_switch) then
          ...
          do j=-derivativeType,derivativeType
             ...
             ! determine energy due to hadronic interaction
             energy(0,j)=hamiltonFunc_offshell(teilchen,outofBounds)
             if(outOfBounds) then
  ...
  subroutine propagate (realTeilchen, pertTeilchen, delta_T, timeStep)
    ...
    If (PredictorCorrector) then
       ...
       if (doReal) then
          Allocate(gradR_real(lbound(realteilchen,dim=1):ubound(realteilchen,dim=1), &
                              lbound(realteilchen,dim=2):ubound(realteilchen,dim=2), &
                              0:3))
          Allocate(gradP_real(lbound(realteilchen,dim=1):ubound(realteilchen,dim=1), &
                              lbound(realteilchen,dim=2):ubound(realteilchen,dim=2), &
                              1:3))
       end if
       Allocate(gradR_pert(lbound(pertteilchen,dim=1):ubound(pertteilchen,dim=1), &
                           lbound(pertteilchen,dim=2):ubound(pertteilchen,dim=2), &
                           0:3))
       Allocate(gradP_pert(lbound(pertteilchen,dim=1):ubound(pertteilchen,dim=1), &
                           lbound(pertteilchen,dim=2):ubound(pertteilchen,dim=2), &
                           1:3))
  ...
  subroutine predictorStep(teilchen,gradP,gradR,delta_T)
    ...
    Do n=lbound(teilchen,dim=1),ubound(teilchen,dim=1)
       Do m=lbound(teilchen,dim=2),ubound(teilchen,dim=2)
          ...
          Teilchen(n,m)%momentum(1:3)=teilchen(n,m)%momentum(1:3)-delta_T*gradR(n,m,1:3)
          Teilchen(n,m)%momentum( 0 )=teilchen(n,m)%momentum( 0 )+delta_T*gradR(n,m,0)
          Teilchen(n,m)%position(1:3)=teilchen(n,m)%position(1:3)+delta_T*gradP(n,m,1:3)
          Teilchen(n,m)%velocity(1:3)=gradP(n,m,1:3)
  ...
  subroutine correctorStep(teilchen,gradP,gradR,delta_T)
    ...
    Do n=lbound(teilchen,dim=1),ubound(teilchen,dim=1)
       Do m=lbound(teilchen,dim=2),ubound(teilchen,dim=2)
          ...
          !
          teilchen(n,m)%momentum(1:3)=teilchen(n,m)%momentum(1:3)-delta_T*(-gradR(n,m,1:3)+grad_R_Predictor(1:3))/2.
          teilchen(n,m)%momentum( 0 )=teilchen(n,m)%momentum( 0 )+delta_T*(-gradR(n,m, 0 )+grad_R_Predictor( 0 ))/2.
          teilchen(n,m)%position(1:3)=teilchen(n,m)%position(1:3)+delta_T*(-gradP(n,m,1:3)+grad_P_Predictor(1:3))/2.

          ! (4) Save velocity
          teilchen(n,m)%velocity(1:3)=(gradP(n,m,1:3)+grad_P_Predictor(1:3))/2.
